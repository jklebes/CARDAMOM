...
!       tot_exp = sum((((M_NEE(DATAin%nbepts(1:DATAin%nnbe))+M_FLUXES(DATAin%nbepts(1:DATAin%nnbe),24)) &
...
!                    M_FLUXES(DATAin%nbepts(1:DATAin%nnbe),24)) &
!              / dble(DATAin%nnbe)
...
!        tot_exp = sum( (((M_NEE(DATAin%nbepts(1:DATAin%nnbe))+M_FLUXES(DATAin%nbepts(1:DATAin%nnbe),24)-model) - &
...
!       tot_exp = sum(((M_FLUXES(DATAin%Evappts(1:DATAin%nEvap),46)-DATAin%Evap(DATAin%Evappts(1:DATAin%nEvap))) &
...
!       tot_exp = sum(((M_FLUXES(DATAin%Firepts(1:DATAin%nFire),24)-DATAin%Fire(DATAin%Firepts(1:DATAin%nFire))) &
...
!    endif
    ...
    if (DATAin%nFire > 0) then
        ...
        do y = 1, DATAin%nos_years
           ...
           if (sum(sub_time) > 0d0) then
               ...
               model = sum(M_FLUXES(s:f,24)*sub_time) / sum(sub_time)
               unc   = sqrt(sum((sub_time*DATAin%Fire_unc(s:f))**2)) / sum(sub_time)
               ! Update the likelihood score with the mean bias
               likelihood = likelihood - (((model - obs) / unc) ** 2)
               ! Determine the anomalies based on substraction of the annual mean
               ! Greater information would come from breaking this down into annual estimates
               likelihood = likelihood - sum( (sub_time*(((M_FLUXES(s:f,24)-model) - &
    ...
    if (DATAin%nClit_stock > 0) then
       ...
       !          * 0.5d0
       mid_state = sum(M_FLUXES(:,9)+M_FLUXES(:,28)+M_FLUXES(:,29)+ & 
                       M_FLUXES(:,34)+M_FLUXES(:,35))
       mid_state = (mid_state / &
                   (mid_state + sum(M_FLUXES(:,10)+M_FLUXES(:,30)+M_FLUXES(:,36)))) & 
                 * M_POOLS(:,4)
       !mid_state = (sum(M_FLUXES(:,9))/sum(M_FLUXES(:,9)+M_FLUXES(:,10))) &
       !          * M_POOLS(:,4)
    ...
    if (DATAin%nCsom_stock > 0) then
       ...
       mid_state = ( M_POOLS(1:DATAin%nodays,5) + M_POOLS(2:(DATAin%nodays+1),5) ) &
                 * 0.5d0
       ! Vectorised version of loop to estimate cost function
...
!        tot_exp = sum(M_FLUXES(:,46)) / sum(DATAin%MET(7,:) * 86400d0)
...
!        likelihood = likelihood-tot_exp
        ...
        do y = 1, DATAin%nos_years
           ...
           if (sum(sub_time) > 0d0) then
               ...
               ! Determine the anomalies based on substraction of the annual mean
               ! Greater information would come from breaking this down into annual estimates
               tot_exp = tot_exp + sum( (sub_time*(((M_NEE(s:f)+M_FLUXES(s:f,24)-model) - &
                                                    (DATAin%NBE(s:f) - obs)) / unc))**2 )
           end if ! sum(sub_time) > 0
        end do ! loop years
    ...
    if (DATAin%nFire > 0) then
        ...
        do y = 1, DATAin%nos_years
           ...
           if (sum(sub_time) > 0d0) then
               ...
               ! Determine the anomalies based on substraction of the annual mean
               ! Greater information would come from breaking this down into annual estimates
               tot_exp = tot_exp + sum( (sub_time*(((M_FLUXES(s:f,24)-model) - &
                                                    (DATAin%Fire(s:f) - obs)) / unc))**2 )
           end if ! sum(sub_time) > 0
        end do ! loop years
        ! Update the likelihood score with the anomaly estimates
