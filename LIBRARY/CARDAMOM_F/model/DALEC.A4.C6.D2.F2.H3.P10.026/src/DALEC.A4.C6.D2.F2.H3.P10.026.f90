...
!    rfac = 0.5d0 ; rfac(5) = 0.1d0 ; rfac(6) = 0d0
    ...
    do n = start, finish
       ...
       call compute_canopy_cohort_averages(max_leaf_cohorts,leaf_cohorts, &
                                           canopy_NUE_rel,DIAGS(n,32),DIAGS(n,33),DIAGS(n,35))
       ...
       call plant_natural_turnover(nodays, n, days_per_step,              & ! time related
                                   ...
                                   pars(16), pars(14),                    & ! potential foliar loss for NCCE trend
       ...
       call update_and_shed_cohorts(days_per_step, FLUXES(n,1), POOLS(n,2), &
                                    ...
                                    pars(14), pars(15), pars(16), FLUXES(n,10), &
                                    cohort_litter_flux, resorb_flux, env_litter_out)
...
!    endif
  ...
  subroutine plant_allocation(nopools,time,                              &
       ...
       double precision, intent(in) :: time, & ! number of days in current time step
                       ...
                       alloc_labile_gCm2day, & ! GPP-Rm (gC/m2/day)
       ...
       if (available_labile > 0d0) then
           ...
           ! Labile to wood rate (gC.m-2.day-1)
           ...
           if (alloc_leaf > 0d0 .and. ncce_gradient > 0d0) then
               ...
               ! Declining gradient, we won't grow leaves
               alloc_leaf = 0d0
           end if ! alloc_leaf > 0
           ...
           ! Wood growth is not essential and should be bound by the maximum rate of GPP-less maintenance respiration.
           alloc_wood = min(alloc_labile_gCm2day,alloc_wood)
  ...
  subroutine plant_natural_turnover(nodays,step,time,                       & ! time related
                                    ...
                                    cmi_ncce_k50,                           & ! MM function of NCCE driven leaf fall
       ...
       if (foliage > 0d0) then
           ...
           if (available_labile <= 0d0) then
               ...
               C_starvation_loss = abs(available_labile*days_per_step_1) / C_starvation_loss
           else 
               C_starvation_loss = 0d0
           end if

           ! Estimate canopy level mortality, a function of the net canopy carbon export (NCCE). 
           ! These are:
           ! 1) A Michaelis-Menten function of NCCE gCgCleaf set to add to turnover once NCCE is negative.
           ! 2) Testing whether a smaller canopy will improve the NCCE per gC/m2/day
           ...
           if (ncce_gCgCday(step) < 0d0) then
               cmi = ncce_gCgCday(step) / (ncce_gCgCday(step) + cmi_ncce_k50)
           else
...
!    logistic_func = (1d0+exp(max_growth*(max_growth_point-invar)))**(-1d0)
  ...
  subroutine initialise_cohorts(foliage, start_doy, r_opp, leaf_age_ref, &
    ...
    do k = 1, max_leaf_cohorts
        ...
        leaf_cohorts(k)%ncce_threshold = 0d0 ! economic shedding threshold (gC gC-1 m-2 d-1)
        leaf_cohorts(k)%profit_target  = 0d0 ! Target for cumulative profit (gC gC-1 m-2 LL)
        leaf_cohorts(k)%ncce_average   = 0d0 ! mean annual cohort level ncce (gC/m2/day)
    ...
    do y = 1, N_years
        ...
        do m = 1, 12
            ...
            leaf_cohorts(k)%profit_target = leaf_cohorts(k)%Cc * r_opp
            leaf_cohorts(k)%ncce_threshold= (leaf_cohorts(k)%Cc + (leaf_cohorts(k)%profit_target)) / max(leaf_age_ref, 1d0)
  ...
  subroutine create_cohort(dCf, dCc, r_opp, leaf_age_ref)
    ...
    if (slot > 0) then
        ...
        leaf_cohorts(slot)%profit_target = dCc * r_opp
        leaf_cohorts(slot)%ncce_threshold= (dCC + (leaf_cohorts(slot)%profit_target)) / max(leaf_age_ref, 1d0)        
        leaf_cohorts(slot)%ncce_average  = 0d0 ! mean annual cohort level ncce (gC/m2/day)        
  ...
  subroutine update_and_shed_cohorts(time, gpp_gCm2day, foliage, &
                                     ...
                                     cmi_ncce_k50, r_opp, pot_foliar_fall_fraction, &
                                     env_litter_flux_gCm2day, cohort_litter_flux, &
    ...
    double precision, intent(in)  :: time, & ! time step (days)
                             ...
                             cmi_ncce_k50, & ! Cohort NCCE at which turnover is 50 % of potential turnover rate
                 pot_foliar_fall_fraction, & ! Potential foliar turnover fraction (/day)
    ...
    do k = 1, max_leaf_cohorts
        ...
        leaf_cohorts(k)%ncce_average = ((leaf_cohorts(k)%ncce_average * (365.25d0-time)) + (ncce_k * time)) * 0.002737851d0
        ! Estimate canopy mortality, a function of the net canopy carbon export (NCCE). 
        ! 1) A Michaelis-Menten function of NCCE gCgCleaf set to add to turnover once NCCE is negative.
        ! 2) Loss permitted to occur if the current cohort ncce is negative and the rolling annual average (i.e. sustained losses)
        !   or the cohort has exceeded its target profit. Thus we are either losing significantly and sustained, or prioritising removal
        !   of older less relatively profiable cohorts.
        if (ncce_k < 0d0 .and. & 
           (leaf_cohorts(k)%ncce_average < 0d0 .or. leaf_cohorts(k)%cum_profit > leaf_cohorts(k)%profit_target)) then
            remaining_env_loss = remaining_env_loss &
                               + (time * leaf_cohorts(k)%Cf * pot_foliar_fall_fraction * (ncce_k / (ncce_k + cmi_ncce_k50)))
        end if            
