...
int EDC1_LU_FIRES_HBV(double const pars[], struct EDCDIAGNOSTIC *EDCD, double meantemp, double meanrad)
{
...
/* JFE commented out for tests with bucket
...
int EDC2_LU_FIRES_HBV(double const pars[], double *MET, double *LAI, double *NEE, double *POOLS,double *FLUXES, PARAMETER_INFO PI, DATA D, struct EDCDIAGNOSTIC *EDCD, double meantemp)
{
...
double fin_fout_lim = 0.05; // get the limit of Fin/Fout ratio // JFE added 16/11/16

//printf("nostep_peryear %i ", nostep_peryear);
//printf("%d",nodays);

int poolid[6] = {0,1,2,3,4,5}; // define pools to check
int pid=0;
