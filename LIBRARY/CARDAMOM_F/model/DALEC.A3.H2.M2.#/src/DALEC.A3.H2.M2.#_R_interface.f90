...
subroutine rdalec17(output_dim,aNPP_dim,MTT_dim,SS_dim,fire_dim &
                   ,met,pars &
                   ,out_var1,out_var2,out_var3,out_var4,out_var5 &
                   ,lat,nopars,nomet,nofluxes,nopools,pft &
                   ,nodays,nos_years,deltat,nos_iter,soil_frac_clay_in &
                   ,soil_frac_sand_in)
...
!endif
     ...
     out_var1(i,1:nodays,56) = ivpd(1:nodays)           ! GSI wSWP component (0-1)
     ...
     where (POOLS(1:nodays,6) > 0d0) ! protection against NaN from division by zero
            pool_hak = 0 
            tmp = ((FLUXES(1:nodays,14) + fire_emiss_som + &
                    fire_litter_som + harvest_extracted_som) &
                  / POOLS(1:nodays,6))
     end where
