#include <math.h>
#include "EDCs.c"
#include "../src/DALEC_CDEA_LU_FIRES_LIU.c" // model
#include "../src/DALEC_CDEA_LU_FIRES_LIU_PARS.c" // parameter info
...
double likelihood(DATA D)
{
    ...
    {        
        for (n=0;n<D.nCabgb_stock;n++)
//        for (n=0;n<D.nodays;n++)
        {
            dn = D.Cabgb_stockpts[n];
    //        printf("%d %d %f\n", n, dn, D.Cabgb_stock[dn]);
//            dn=D.Cabgb_stockpts[n];
//            double M_Cabgb = D.M_POOLS[(dn*D.nopools)+0]+D.M_POOLS[(dn*D.nopools)+1]+D.M_POOLS[(dn*D.nopools)+2]+D.M_POOLS[(dn*D.nopools)+3];
//            tot_exp=tot_exp+pow((M_Cabgb-D.Cabgb_stock[dn])/(D.Cabgb_stock[dn]*0.50),2);
            tot_exp=tot_exp+pow((D.M_ABGB_ANN[n]-D.Cabgb_stock[dn])/(D.Cabgb_stock[dn]*.5),2);
            //printf("%d obs:%f:  \n", n, D.Cabgb_stock[dn]);      
        }
   // printf("obs:%f - mod: %f \n", D.Cabgb_stock[dn], D.M_ABGB_ANN[n-1]);
    }
...
double model_likelihood(DATA D, PARAMETER_INFO PI, double *PARS)
{
    ...
    int EDC,n;
    double P=0,P_p;
    int tt, yy; // iterator for mean annual pools
    double tmp_abgb = 0;

//    for (n=0; n<PI.npars; n++){
   // printf("param value passed EDCs0  %f %f\n", PI.parini[0], PARS[0]);
    // check first set of EDCs: parameter values
    EDC=pow(EDC1_LU_FIRES(PARS, &EDCD, D.meantemp, D.meanrad), D.EDC);
//    for (n=0; n<PI.npars; n++){
    //printf("param value passed EDCs1  %f %f\n", PI.parini[0], PARS[0]);
    P=P+log((double)EDC);
 //   printf("hi from model_likelihood1 l119: %d\n", EDC);
...
//        for (n=0; n<PI.npars; n++){
       // printf("param value passed before EDCs2c %f %f\n", PI.parini[0], PARS[0]);
        ...
        // EDC2 check
        EDC=EDC2_LU_FIRES(PARS, D.MET, D.M_LAI, D.M_NEE, D.M_POOLS, D.M_FLUXES, PI, D, &EDCD, D.meantemp);
        EDC=pow(EDC,D.EDC);
       // printf("hi from model_likelihood l 142: %d\n", EDC);
        // printf("EDC2: %i; ",EDC);


        // LIKELIHOOD: -inf if EDC=0
        P=P+log((double)EDC);
        // if EDCs are passed, calculate mean annual ABGB abd then likelihood
        if (EDC==1)
        {
            //iterate time steps
            ...
            tmp_abgb = 0;
            yy = 0;
            for (tt=0;tt<D.nodays;tt++)
            {
                tmp_abgb = tmp_abgb+D.M_POOLS[D.nopools*tt+0]+D.M_POOLS[D.nopools*tt+1]+D.M_POOLS[D.nopools*tt+2]+D.M_POOLS[D.nopools*tt+3];     
                
                // if last month of the year, save value
                if ( tt % 12 == 11)
                {
                //    printf("calculate modulo abgb: %d\n", EDC);
                    D.M_ABGB_ANN[yy] = tmp_abgb/12;                   
                //    printf("year %d %f %f\n", yy, D.M_ABGB_ANN[yy], tmp_abgb);
                    tmp_abgb = 0; yy = yy+1;
                }
                
            //    printf("tt %d\n",tt);
            }
           // printf("%d %d %f %f \n",yy,tt,D.M_ABGB_ANN[yy-1],D.M_POOLS[D.nopools*tt+3]);
            P=P+likelihood(D);
        }
    }
...
double edc_model_likelihood(DATA D, PARAMETER_INFO PI, double *PARS)
{
    ...
    EDC=EDC1_LU_FIRES(PARS, &EDCD, D.meantemp, D.meanrad);
   ...
   // printf("EDC1 %d\n",EDC);
    ...
    EDC=EDC*EDC2_LU_FIRES(PARS, D.MET, D.M_LAI, D.M_NEE, D.M_POOLS, D.M_FLUXES, PI, D, &EDCD, D.meantemp);
...
void find_edc_initial_values(PARAMETER_INFO *PI, DATA D)
{
    ...
    for (parid = 0; parid < PI->npars; parid++) {PI->parini[parid]=MCOUT_init.best_pars[parid]; PI->parfix[parid]=0;}//printf("%d %f\n",parid, PI->parini[parid]);}
}
