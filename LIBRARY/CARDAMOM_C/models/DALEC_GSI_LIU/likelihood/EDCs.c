...
int EDC1_LU_FIRES_GSI(double const pars[], struct EDCDIAGNOSTIC *EDCD, double meantemp, double meanrad)
{
    ...
    /*  GSI specific checks
        Tmn,min < Tmn,max */
    if ((EDC==1 || DIAG==1) && (pars[13]>pars[14])){EDC=0;EDCD->PASSFAIL[6]=0;}

    //  Photoperiod,min < Photoperiod,max
    if ((EDC==1 || DIAG==1) && (pars[15]>pars[23])){EDC=0;EDCD->PASSFAIL[7]=0;}

    //  VPD,min < VPD,max
    if ((EDC==1 || DIAG==1) && (pars[24]>pars[25])){EDC=0;EDCD->PASSFAIL[8]=0;}
...
int EDC2_LU_FIRES_GSI(double const pars[], double *MET, double *LAI, double *NEE, double *POOLS,double *FLUXES, PARAMETER_INFO PI, DATA D, struct EDCDIAGNOSTIC *EDCD, double meantemp)
{
...
if ((EDC==1 || DIAG==1) && (MPOOLS[1]>MPOOLS[2]*5 || MPOOLS[1]*5<MPOOLS[2])){EDC=0;EDCD->PASSFAIL[8]=0; }
...
for (n=0;n<nopools;n++)
{
    if (EDC==1 || DIAG==1) 
    {
        ...
        if (fabs(-log(2)/C)<365.25*cyears && C<0 && finite(C)){EDC=0;}
        if (EDC==0){EDCD->PASSFAIL[11]=0;}
    }
}
