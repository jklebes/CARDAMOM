...
! of DALEC.A1.C2.D2.F2.H2.P10.R2 as a function of observations and ecological dynamical constraints.
  !
...
module model_likelihood_module
  ...
  ! which to make open
  public::  model_likelihood, edc_model_likelihood, &
...
!print*,"sanity_check: carbon_model done 2"
  ...
  subroutine assess_EDC1(PARS, npars, meantemp, meanrad, EDC1)
    ...
    ! Minimum and optimum canopy growth temperatures
...
!    end if
    ...
    ! The optimum temperature for canopy growth should not be less than the mean temperature at the site
    ! This might not hold true for tropical systems..
    if ((EDC1 == 1 .or. DIAG == 1) .and. (pars(24) < meantemp)) then
...
!    endif
    ...
    ! Similarly, the initial mean NUE parameter (pars(44)) should not be too far away from the
    ! actual estimate nor should it be greater than the optimium (pars(42))
    if ((EDC2 == 1 .or. DIAG == 1) .and. (NUE_mean_annual > pars(42)*1.25d0 .or. NUE_mean_annual < pars(42)*0.75d0 .or. &
                                          pars(44) > pars(42)) ) then
    ...
    if (EDC2 == 1 .or. DIAG == 1) then
       ...
       biomass_root = (M_POOLS(1:DATAin%nodays,3) + (M_POOLS(1:DATAin%nodays,3) * pars(29))) * 2d0
...
!        if (abs(Rs-in_out_lab) > 0.1d0 .or. abs(log(in_out_lab)) > EQF10) then
        if (abs(abs(log(in_out_lab_yr1) - log(in_out_lab_yr2))) > C_etol .or. &
...
!        if (abs(Rs-in_out_fol) > 0.1d0 .or. abs(log(in_out_fol)) > EQF10) then
        if (abs(abs(log(in_out_fol_yr1) - log(in_out_fol_yr2))) > C_etol .or. &
...
!        if (abs(Rs-in_out_root) > 0.1d0 .or. abs(log(in_out_root)) > EQF10) then
        if (abs(abs(log(in_out_root_yr1) - log(in_out_root_yr2))) > C_etol .or. &
...
!        if (abs(Rs-in_out_wood) > 0.1d0 .or. abs(log(in_out_wood)) > EQF10) then
        if (abs(abs(log(in_out_wood_yr1) - log(in_out_wood_yr2))) > C_etol .or. &
...
!        if (abs(Rs-in_out_lit) > 0.1d0 .or. abs(log(in_out_lit)) > EQF10) then
        if (abs(abs(log(in_out_lit_yr1) - log(in_out_lit_yr2))) > C_etol .or. &
...
!        if (abs(Rs-in_out_som) > 0.1d0 .or. abs(log(in_out_som)) > EQF10) then
        if (abs(abs(log(in_out_som_yr1) - log(in_out_som_yr2))) > etC_etolol .or. &
...
!        if (abs(Rs-in_out_litwood) > 0.1d0 .or. abs(log(in_out_litwood)) > EQF10) then
        if (abs(abs(log(in_out_litwood_yr1) - log(in_out_litwood_yr2))) > C_etol .or. &
...
!print*,"likelihood: GPP done"
    ...
    if (DATAin%nEvap > 0) then
       tot_exp = sum(((M_FLUXES(DATAin%Evappts(1:DATAin%nEvap),19)-DATAin%Evap(DATAin%Evappts(1:DATAin%nEvap))) &
...
!print*,"likelihood: Fire done"
    ...
    if (DATAin%nCfol_stock > 0) then
       ...
       tot_exp = sum(( (mid_state(DATAin%Cfol_stockpts(1:DATAin%nCfol_stock)) &
                       -mid_state(DATAin%Cfol_stockpts(1:DATAin%nCfol_stock)))&
    ...
    if (DATAin%nCwood_stock > 0) then
       ...
       tot_exp = sum(( (mid_state(DATAin%Cwood_stockpts(1:DATAin%nCwood_stock)) &
                       -mid_state(DATAin%Cwood_stockpts(1:DATAin%nCwood_stock)))&
    ...
    if (DATAin%nCroots_stock > 0) then
       ...
       tot_exp = sum(( (mid_state(DATAin%Croots_stockpts(1:DATAin%nCroots_stock)) &
                       -mid_state(DATAin%Croots_stockpts(1:DATAin%nCroots_stock)))&
    ...
    if (DATAin%otherpriors(1) > -9998) then
        tot_exp = sum(M_FLUXES(:,3)) / sum(M_FLUXES(:,1))
    ...
    if (DATAin%otherpriors(2) > -9998) then
        tot_exp = (M_POOLS(1,8) * 1d-3) / layer_thickness(1) ! convert mm -> m3/m3
    ...
    if (DATAin%otherpriors(4) > -9998) then
        tot_exp = sum(M_FLUXES(:,19)) / sum(DATAin%MET(7,:) * 86400d0)
    ...
    if (DATAin%otherpriors(5) > -9998) then
        ...
        output = sum(M_POOLS(:,4) / (M_FLUXES(:,11)+fire_loss_wood))
