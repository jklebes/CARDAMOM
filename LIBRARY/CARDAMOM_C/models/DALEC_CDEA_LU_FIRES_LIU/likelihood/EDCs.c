...
int EDC1_LU_FIRES(double const pars[], struct EDCDIAGNOSTIC *EDCD, double meantemp, double meanrad)
{
    ...
    if ((EDC==1 || DIAG==1) && ((flab<0.01) || (flab > 0.5))){EDC=0;EDCD->PASSFAIL[40]=0;}
    if ((EDC==1 || DIAG==1) && ((ffol<0.01) || (ffol > 0.5))){EDC=0;EDCD->PASSFAIL[41]=0;}
    if ((EDC==1 || DIAG==1) && ((froot<0.01) || (froot > 0.5))){EDC=0;EDCD->PASSFAIL[42]=0;}
    if ((EDC==1 || DIAG==1) && ((fwood<0.01) || (fwood > 0.5))){EDC=0;EDCD->PASSFAIL[43]=0;}
...
int EDC2_LU_FIRES(double const pars[], double *MET, double *LAI, double *NEE, double *POOLS,double *FLUXES, PARAMETER_INFO PI, DATA D, struct EDCDIAGNOSTIC *EDCD, double meantemp)
{
...
int noyears=floor(nodays*deltat/365); if (noyears == 0) {noyears = 1;}
double G=0.1;
int y;
...
if ((EDC==1 || DIAG==1) && (MAPOOL[noyears-1]/MAPOOL[0]>(1+G*(double)noyears))){EDC=0;EDCD->PASSFAIL[9]=0;}// printf("failed no. 9");}
...
/* EDC 12 - SOM steady-state attractor proximity attractor proximity
 * Csom inputs: (10,14,26,27)
 * Csom outputs: (13,22)-1 */
Fin=FT[10]+FT[14]+FT[26]+FT[27];
Fout=FT[13]+FT[22];
if ((EDC==1 || DIAG==1) && (fabs(log(Fin/Fout))>log(EQF)))
{EDC=0;EDCD->PASSFAIL[12-1]=0;}// printf("failed no. 11");}
//extract pool sizes in jan
Sproxjanmean=0;
for (yearid=0;yearid<noyears;yearid++)
{
    //printf(" %d %f", yearid, POOLS[(12*yearid)+5]);
    Sproxjanmean = Sproxjanmean+POOLS[(12*yearid)+5];
}
Sproxjanmean = Sproxjanmean/noyears;
Sproxjan = (Fin/Fout)*(Sproxjanmean/POOLS[5]); //eq. S5
if ((EDC==1 || DIAG ==1) && (fabs((Fin/Fout)-Sproxjan) > 0.05)) {EDC=0;EDCD->PASSFAIL[10]=0;}

/*print some stuff*/
if (psw==1){
printf("****\n");
printf("Csom Fin = %f,Fout = %f\n",Fin,Fout);
...
/*EDC 13 - LIT  -steady-state attractor proximity*/
/*Litter inputs: 9,11,23,24,25*/
/*litter outputs:  12,14,21,27*/
Fin=FT[9]+FT[11]+FT[23]+FT[24]+FT[25];
Fout=FT[12]+FT[14]+FT[21]+FT[27];
if ((EDC==1 || DIAG==1) && (fabs(log(Fin/Fout))>log(EQF)))
{EDC=0;EDCD->PASSFAIL[13-1]=0;}// printf("failed no. 12");}
//extract pool sizes in jan
Sproxjanmean=0;
for (yearid=0;yearid<noyears;yearid++)
{
    Sproxjanmean = Sproxjanmean+POOLS[(12*yearid)+4];
}
Sproxjanmean = Sproxjanmean/noyears;
//printf("lit %f ",Sproxjanmean);
Sproxjan = (Fin/Fout)*(Sproxjanmean/POOLS[4]); //eq. S5
if ((EDC==1 || DIAG ==1) && (fabs((Fin/Fout)-Sproxjan) > 0.05)) {EDC=0;EDCD->PASSFAIL[11]=0;}

/*print some stuff*/
if (psw==1){
printf("Clit Fin = %f,Fout = %f\n",Fin,Fout);
printf("fabs(log(Fin/Fout)) = %f\n",fabs(log(Fin/Fout)));
printf("log(EQF) = %f\n",log(EQF));
printf("****");}
/*done printing stuff*/
...
/*EDC 14 - WOO  -steady-state attractor proximity*/
/*Wood inputs   6 */
/*Wood outputs  10,20,26  */
Fin=FT[6];
Fout=FT[10]+FT[20]+FT[26];
if ((EDC==1 || DIAG==1) && (fabs(log(Fin/Fout))>log(EQF)))
{EDC=0;EDCD->PASSFAIL[14-1]=0;}// printf("failed no. 13");}
//extract pool sizes in jan
Sproxjanmean=0;

for (yearid=0;yearid<noyears;yearid++)
{
    Sproxjanmean = Sproxjanmean+POOLS[(12*yearid)+3];
}
Sproxjanmean = Sproxjanmean/noyears;
//printf("woo %f ",Sproxjanmean);
Sproxjan = (Fin/Fout)*(Sproxjanmean/POOLS[3]); //eq. S5
//printf("%f %f %f\n", Sproxjan,(Fin/Fout),fabs((Fin/Fout)-Sproxjan));
if ((EDC==1 || DIAG ==1) && (fabs((Fin/Fout)-Sproxjan) > 0.05)) {EDC=0;EDCD->PASSFAIL[12]=0;}

/*print some stuff*/
if (psw==1){
printf("****\n");
printf("Cwoo Fin = %f,Fout = %f\n",Fin,Fout);
printf("fabs(log(Fin/Fout)) = %f\n",fabs(log(Fin/Fout)));
printf("log(EQF) = %f\n",log(EQF));
printf("****\n");}
/*done printing stuff*/
...
/*EDC 15 - ROO  -steady-state attractor proximity*/
/*Root inputs   5 */
/*Root outputs  11,19,25 */
Fin=FT[5];
Fout=FT[11]+FT[19]+FT[25];
if ((EDC==1 || DIAG==1) && (fabs(log(Fin/Fout))>log(EQF)))
{EDC=0;EDCD->PASSFAIL[14-1]=0;}// printf("failed no. 14");}
//extract pool sizes in jan
Sproxjanmean=0;
for (yearid=0;yearid<noyears;yearid++)
{
    Sproxjanmean = Sproxjanmean+POOLS[(12*yearid)+2];
}
Sproxjanmean = Sproxjanmean/noyears;
Sproxjan = (Fin/Fout)*(Sproxjanmean/POOLS[2]); //eq. S5
if ((EDC==1 || DIAG ==1) && (fabs((Fin/Fout)-Sproxjan) > 0.05)) {EDC=0;EDCD->PASSFAIL[13]=0;}

/*print some stuff*/
if (psw==1){
printf("****\n");
printf("Croo Fin = %f,Fout = %f\n",Fin,Fout);
printf("fabs(log(Fin/Fout)) = %f\n",fabs(log(Fin/Fout)));
printf("log(EQF) = %f\n",log(EQF));
printf("****\n");}
/*done printing stuff*/
