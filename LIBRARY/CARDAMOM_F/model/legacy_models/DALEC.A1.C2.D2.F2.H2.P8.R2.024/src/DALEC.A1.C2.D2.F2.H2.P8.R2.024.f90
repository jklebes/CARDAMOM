...
! This file contains the source code of DALEC.A1.C2.D2.F2.H2.P8.R2
!
...
!    rfac = 0.5d0 ; rfac(5) = 0.1d0 ; rfac(6) = 0d0
    ...
    do n = start, finish
       ...
       call plant_canopy_phenology(nodays, cgi_ncce_lag_step, n, deltat(n),               & ! Timing
                                   ...
                                   pars(14), pars(45), pars(49), pars(50),                & !
                                   pars(51), pars(52), pars(12), pars(5),                 & ! 
...
!    endif
  ...
  subroutine plant_canopy_phenology(nodays, cgi_ncce_lag_step, step, time,                 & ! Timing
                                    cgi_temperature_coef_1, cgi_temperature_coef_2,        & ! CGI / CMI parameters
                                    ...
                                    cmi_temperature_coef_1, cmi_temperature_coef_2,        & !
       ...
       double precision, intent(in) :: time, & ! number of days in current time step
                    ...
                    cgi_phenology_threshold, & ! CGI gradient threshold above which leaf growth is possible 
                     ...
                     cmi_temperature_coef_1, & ! max temperature for skewed Gaussian mortlaity curve (oC)
                     cmi_temperature_coef_2, & ! min temperature for skewed Gaussian mortality curve (oC)
                     cmi_temperature_coef_3, & ! optimum temperature for mortality (oC)
       ...
       leafT_limit = 0d0 ; leafW_limit = 0d0 ; tmp = 0d0
       ...
       if (ncce_gradient < 0d0) then
           tmp(1) = ncce_gradient / (ncce_gradient + cmi_ncce_gradient_k50)
       end if 
       ! MM function add additinal impact of NCCE if negative
       if (ncce_gCgC(step) < 0d0) then
           tmp(2) = ncce_gCgC(step) / (ncce_gCgC(step) + cmi_ncce_k50)
       end if 
       ! Estimate the temperature limitation on foliage mortality
       tmp(3) = 1d0 - opt_max_scaling(cmi_temperature_coef_1,cmi_temperature_coef_2 &
