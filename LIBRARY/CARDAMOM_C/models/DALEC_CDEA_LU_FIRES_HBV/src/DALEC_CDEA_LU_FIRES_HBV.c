...
double *LAI, double *NEE, double *FLUXES, double *POOLS) {
    ...
    // water storage is a fraction of fc
    POOLS[6]=pars[26]*pars[24];
    ...
    double lp = pars[23]; // lp to set threshold for evapotranspiration
    double fc = pars[24]; // field capacity: max water storage
    double beta = pars[25]; // beta to scale etpot
    double dQdP; // to store intermediary values

    // dummy terms to store daily GPP, ET, WS, and monthly Vs and ET
    double dummyGPP,dummyET,dummySM, dummyQ, dummyQ_sum, dummyET_sum;
    ...
    for ( n=0; n < nr; n++) {
        ...
        */
        ...
        double ETpot = MET[m+10]; //adjust ET from ref crop to current vegetation
        ...
        for (dd=0;dd<timestep;dd++) {
            ...
            // fraction of rainfall that is outflow is function of ratio of WS to fc 
            dQdP = pow(dummySM/fc,beta);
            //dQdP = dummySM/fc;  
            // if rain + soil moisture > field capacity, all rain is outflow
            if(dQdP>1) {dQdP=1;};
            // update soil moisture and outflow
            dummySM = dummySM + (1-dQdP) * MET[m+9];
            dummyQ = dQdP * MET[m+9];

            // if SM is greater than lp*fc, then ETact=ETpot
            if (0.5*(dummySM+POOLS[p+6])<(lp*fc)) 
                {
                    dummyET = ETpot*0.5*(dummySM+POOLS[p+6])/(lp*fc);
                }
            else
                {
                    dummyET = ETpot;
            ...
            // update the dummySM for multiday time stepping
            dummySM = dummySM-dummyET;
            ...
            //save daily outflow to get monthly sum
            dummyQ_sum  = dummyQ_sum + dummyQ;
        }

        // save states and fluxes after iterating time steps
        POOLS[nxp+6] = dummySM;
        FLUXES[f+0] = dummyGPP/timestep;
     //   printf("GPP %f\n",dd,FLUXES[f+0]);
        FLUXES[f+28] = dummyET_sum/timestep;
        FLUXES[f+29] = dummyQ_sum/timestep;
