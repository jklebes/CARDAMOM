    ...
    if (DATAin%nlai > 0) then
       do n = 1, DATAin%nlai
         ...
         if (vars%M_LAI(dn) >= 0.) then
             ! note that division is the uncertainty
             tot_exp=tot_exp+(log(max(1e-6,vars%M_LAI(dn))/max(1e-6,DATAin%LAI(dn)))/log(DATAin%LAI_unc(dn)))**2
         else
...
!         tot_exp=tot_exp+((vars%M_POOLS(dn,4)-DATAin%Cwood_stock(dn))/(DATAin%Cwood_stock(dn)*0.20))**2.
    ...
    if (DATAin%nCroots_stock > 0) then
       do n = 1, DATAin%nCroots_stock
         ...
         ! note that division is the uncertainty
         tot_exp=tot_exp+(((vars%M_POOLS(dn,3)+vars%M_POOLS(dn,7))-DATAin%Croots_stock(dn)) &
