...
!    print*,"carbon_model: "
    ...
    if (.not.allocated(mV%deltat_1)) then
        ...
        mV%total_water_flux = 0d0; mV%water_flux_mmolH2Om2s = 0d0
        mV%soil_waterfrac = mV%soil_waterfrac_initial
        mV%SWP = mV%SWP_initial
...
!    rfac = 0.5d0; rfac(5) = 0.1d0; rfac(6) = 0d0
    ...
    do n = start, finish
       ...
       mV%wSWP_time(n) = mV%wSWP; mV%rSWP_time(n) = mV%rSWP; mV%root_depth_time(n) = mV%root_reach
       ...
       FLUXES(n, 48) = mV%uptake_fraction(1)  ! transpiration extracted from 1st rooting layer (the soil surface)
       FLUXES(n, 49) = mV%uptake_fraction(2)  ! transpiration extracted from 2nd rooting layer (dynamic 2nd layer)
...
!    print*,"carbon_model: done"
  ...
  subroutine calculate_stomatal_conductance (mV)
    ...
    if (mV%aerodynamic_conductance > vsmall .and. mV%total_water_flux > vsmall .and. mV%leafT > Vc_minT) then

        ! Determine potential water flow rate (mmolH2O.m-2.s-1)
        mV%max_supply = mV%total_water_flux
...
!        potential_conductance = (aerodynamic_conductance*leaf_canopy_wind_scaling) / (denom/psych)
        ...
        denom = (denom / (mV%lambda*mV%max_supply*mmol_to_kg_water)) - mV%slope
...
! NOTE: Can something about snow reflectance be added here to reduce radiation available for subsequent absorption/reflectance, and just reflect back to sky?
    ...
    if (mV%snow_storage > 0d0) then
        fsnow = 1d0-exp( - mV%snow_storage*1d-2 )  ! fraction of snow cover on the ground
...
!    endif
  ...
  subroutine calculate_Rtot (mV)
    ...
    ! reset water flux
    mV%total_water_flux = 0d0; mV%water_flux_mmolH2Om2s = 0d0; mV%wSWP = 0d0; mV%rSWP = 0d0
...
!    end if  ! abs(balance) > 1d-10
  ...
  subroutine infiltrate(rainfall_in, mV)
    ...
    mV%infiltrated = mV%infiltrated + (mV%waterchange(1) * 1e3)
  ...
  subroutine update_soil_initial_conditions(input_soilwater_frac, mV)
    ...
    mV%soil_waterfrac = mV%field_capacity
    mV%SWP = mV%SWP_initial

    ! If prior value has been given
    if (input_soilwater_frac > -9998d0) then
        ! calculate initial soil water fraction
        mV%soil_waterfrac(1:nos_soil_layers) = input_soilwater_frac !* field_capacity(1:nos_soil_layers)
