...
extract_Cwood_initial<- function(spatial_type,resolution,grid_type,latlon_in,Cwood_initial_all) {
   ...
   if (spatial_type == "grid") {
       ...
       product_res = c(abs(Cwood_initial_all$long[2,1]-Cwood_initial_all$long[1,1]),abs(Cwood_initial_all$lat[1,2]-Cwood_initial_all$lat[1,1]))
       if (grid_type == "wgs84") {
   ...
   average_i = max(1,(i1-radius[1])):min(dim(Cwood_initial_all$biomass_gCm2)[1],(i1+radius[1]))
   average_j = max(1,(j1-radius[2])):min(dim(Cwood_initial_all$biomass_gCm2)[2],(j1+radius[2]))
   # Carry out averaging
