...
subroutine rdaleccdealufires(output_dim,aNPP_dim,MTT_dim,SS_dim,fire_dim &
                            ,met,pars &
  ...
  use CARBON_MODEL_MOD, only: CARBON_MODEL, extracted_C, CiCa_time
  ...
  do i = 1, nos_iter
     ...
     ! Litter (fol+root; /day)
     out_var6(i,4) = sum( ((FLUXES(1:nodays,13)+FLUXES(1:nodays,15)) / (POOLS(1:nodays,5))) * lit_filter) &
                   / dble(nodays-sum(lit_hak))
     ! Soil (/day)
     ...
     do y = 1, nos_years
        ...
        ! Labile
        out_var9(i,1,y) = sum((FLUXES(y_s:y_e,8))*deltat(y_s:y_e))
        ! Foliage
