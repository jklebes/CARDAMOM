...
!
  subroutine CARBON_MODEL(start,finish,met,pars,deltat,nodays,lat,lai,NEE,FLUXES,POOLS &
    ...
    double precision :: root_litter,npp_to_roots,aroot_loss_rate,aroot_gain_rate
   ...
   ! assign climate sensitivities
    ...
    do n = start, finish
      ...
      npp_to_roots = (FLUXES(n,1)-FLUXES(n,3)-FLUXES(n,5))*pars(4)
      ! disused
      FLUXES(n,4) = 0.0
      ! labile allocation to wood
      npp_to_wood = FLUXES(n,1)-FLUXES(n,3)-FLUXES(n,5)-npp_to_roots
      ...
      ! fine root production (assuming that under moisture more roots are
      ! actually needed)
      FLUXES(n,6) = pars(37)*Cwood_labile_release_coef
      ...
      ! wood production
      FLUXES(n,7) = pars(38)*Cwood_labile_release_coef
      ...
      ! root labile pool
      POOLS(n+1,7) = POOLS(n,7) + (npp_to_roots-FLUXES(n,6))*deltat(n)
      ! wood labile pool
      POOLS(n+1,8) = POOLS(n,8) + (npp_to_wood -FLUXES(n,7))*deltat(n)
      ! foliar pool
      ...
      if (met(8,n) > 0.) then
          ...
          POOLS(n+1,7) = max(0.,POOLS(n+1,8)-labile_root_loss)
          POOLS(n+1,8) = max(0.,POOLS(n+1,9)-labile_wood_loss)
          ...
          if (post_harvest_burn(harvest_management) > 0.) then
              ...
              NCFF(6) = POOLS(n+1,7)*post_harvest_burn(harvest_management)*(1-combust_eff(1))*(1-rfac)
              ...
              NCFF(7) = POOLS(n+1,8)*post_harvest_burn(harvest_management)*(1-combust_eff(1))*(1-rfac)
          ...
          POOLS(n+1,7) = max(0., POOLS(n+1,7) - CFF(6) - NCFF(6) )
          POOLS(n+1,8) = max(0., POOLS(n+1,8) - CFF(7) - NCFF(7) )
          ...
          POOLS(n+1,6) = max(0., POOLS(n+1,6) + (soil_loss_with_roots) + (NCFF(4)+NCFF(5)+NCFF(8)))
      ...
      if (met(9,n) > 0.) then
         ...
         NCFF(6) = POOLS(n+1,7)*met(9,n)*(1-combust_eff(1))*(1-rfac)
         !/*WOODLABILE*/
         CFF(7) = POOLS(n+1,8)*met(9,n)*combust_eff(1)
         NCFF(7) = POOLS(n+1,8)*met(9,n)*(1-combust_eff(1))*(1-rfac)
         !/*foliar*/
         ...
         NCFF(8) = POOLS(n+1,9)*met(9,n)*(1-combust_eff(4))*(1-rfac)
         !/*fires as daily averages to comply with units*/
         FLUXES(n,17)=(CFF(1)+CFF(2)+CFF(3)+CFF(4)+CFF(5)+CFF(6)+CFF(7)+CFF(8))/deltat(n)
