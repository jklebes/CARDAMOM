...
int EDC1_LU_FIRES(double const pars[], struct EDCDIAGNOSTIC *EDCD, double meantemp, double meanrad)
{
    ...
    // TOR cwd must be slower than litter
    if ((EDC==1 || DIAG==1) && (pars[24]>(pars[0]+pars[7]))){EDC=0;EDCD->PASSFAIL[44] = 0;}

    // TOR cwd must be faster than wood
    if ((EDC==1 || DIAG==1) && (pars[24]*exp(pars[9]*meantemp)<pars[5])){EDC=0;EDCD->PASSFAIL[45] = 0;}

    // TOR cwd must be faster than som
    //if ((EDC==1 || DIAG==1) && (pars[24]<pars[8])){EDC=0;EDCD->PASSFAIL[46] = 0;}
...
/*labile*/
Fin[0]=FT[4];
Fout[0]=FT[7]+FT[18]+FT[25];
/*foliar*/
Fin[1]=FT[3]+FT[7];
Fout[1]=FT[9]+FT[19]+FT[26];
/*root*/
Fin[2]=FT[5];
Fout[2]=FT[11]+FT[20]+FT[27];
/*wood*/
Fin[3]=FT[6];
Fout[3]=FT[10]+FT[21]+FT[28];
/*litter*/
Fin[4]=FT[9]+FT[11]+FT[16]+FT[25]+FT[26]+FT[27];
Fout[4]=FT[12]+FT[14]+FT[22]+FT[29];
/*som*/
Fin[5]=FT[14]+FT[28]+FT[29]+FT[31];
Fout[5]=FT[13]+FT[23];//+FT[30]; //FT[30]=0
//cwd
Fin[6]=FT[10];
Fout[6]=FT[16]+FT[24]+FT[31];

int pp = 0; //pool iterator
...
int poolid[7] = {0,1,2,3,4,5,6}; // define pools to check
int pid=0;
...
for (pid=0;pid>7;pid++)
{
    ...
    //set lower change in soil 
 //   if (pid == 5) {fin_fout_lim = fin_fout_lim/10.;}
