...
! this file holds a version of the model that is 
!
!--------------------------------------------------------------------
!
...
!f2py intent(hide) :: nopars=size(pars,1), nomet=size(met,1), nofluxes=size(fluxes,2)
!f2py intent(hide) :: nopools=size(pools,2), nodays=size(met,2)
