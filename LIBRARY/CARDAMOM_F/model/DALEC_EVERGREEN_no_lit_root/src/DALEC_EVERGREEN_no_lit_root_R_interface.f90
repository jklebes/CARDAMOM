...
subroutine rdalecevergreennolitroot(output_dim,aNPP_dim,MTT_dim,SS_dim,met,pars &
...
!endif
     ...
     out_var3(i,4) = 0d0
     out_var3(i,5) = pars(5,i) * airt_adj ! som (/day)
     ...
     do y = 1, nos_years
        ...
        out_var5(i,3,y) = sum( ((FLUXES(y_s:y_e,11)+FLUXES(y_s:y_e,20)+FLUXES(y_s:y_e,26)) &
                                / POOLS(y_s:y_e,2)) * wood_filter(y_s:y_e)) / dble(steps_per_year-sum(wood_hak(y_s:y_e)))
        ! Litter (fol+fine root)
        out_var5(i,4,y) = 0d0
        ! Soil
        out_var5(i,5,y) = sum( ((FLUXES(y_s:y_e,13)+FLUXES(y_s:y_e,21)) &
                                / POOLS(y_s:y_e,3)) * som_filter(y_s:y_e)) / dble(steps_per_year-sum(som_hak(y_s:y_e)))
     end do
