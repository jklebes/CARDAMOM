...
!    endif
  ...
  subroutine EDC1_GSI(PARS, npars, meantemp, meanrad, EDC1)
    ...
    if ((EDC1 == 1 .or. DIAG == 1) .and. pars(9) > pars(38) ) then
       EDC1 = 0 ; EDCD%PASSFAIL(3) = 0
    endif
    ...
    ! turnover of cwd (pars(38)) should be slower than that of roots (pars(6))
...
!    endif
  ...
  subroutine EDC2_GSI(npars,nomet,nofluxes,nopools,nodays,deltat &
    ...
    double precision :: mean_pools(nopools), G, decay_coef, meangpp &
                       ,sumgpp, sumnpp, model_living_C, tmp, tmp1 &
...
!    end do ! year loop
    ...
    tmp1 = ((sumcwd/dble(nodays)) / pars(38)) ! the steady state approximation of cwd (gC/m2)
    ...
    if ((EDC2 == 1 .or. DIAG == 1) .and. pars(37) > tmp1*1.5d0) then
       EDC2 = 0 ; EDCD%PASSFAIL(19) = 0
    end if
...
!print*,"EDC2-37",EDC2
        ...
        if (abs(log(in_out_lit)) > EQF1_5) then
...
!       ML_obs_out = ML_obs_out + likelihood_p(PI%npars,DATAin%parpriors,DATAin%parpriorunc,PARS)
