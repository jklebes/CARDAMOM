...
! Subroutine to allow direct interface between DALEC.A1.C2.D2.F2.H1.P7.R2 and the R code
!
...
subroutine rdalec23(output_dim,MTT_dim,SS_dim &
  ...
  use CARBON_MODEL_MOD, only: CARBON_MODEL, wSWP_time &
                             ...
                             ,NCCE, root_depth_time, Rm_from_labile &
                             ,snow_storage_time
...
!endif
     ...
     out_var1(i,1:nodays,63) = NCCE(1:nodays)           ! Net Canopy C export (NCCE; gCm2day)
     ! Photosynthesis / C~water coupling related
     out_var1(i,1:nodays,64) = gs_demand_supply_ratio   ! ratio of evaporative demand over supply
     out_var1(i,1:nodays,65) = gs_total_canopy          ! stomatal conductance (mmolH2O/m2ground/day)
     out_var1(i,1:nodays,66) = canopy_par_MJday_time    ! Canopy absorbed PAR (MJ/m2ground/day)
     out_var1(i,1:nodays,67) = gb_total_canopy          ! boundary conductance (mmolH2O/m2ground/day)
     out_var1(i,1:nodays,68) = cica_time                ! ratio of leaf internal to external CO2
     ! Misc
     out_var1(i,1:nodays,69) = root_depth_time          ! root depth (m)
