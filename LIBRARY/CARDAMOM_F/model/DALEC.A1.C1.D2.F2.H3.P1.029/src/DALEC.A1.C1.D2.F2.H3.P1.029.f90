...
!    print*,"carbon_model: "
    ...
    ! minimum LWP
    minlwp = pars(33) ! minimum LWP (MPa)
    ...
    if (.not.allocated(deltat_1)) then
        ...
        allocate(deltat_1(nodays),wSWP_time(nodays),rSWP_time(nodays),gs_demand_supply_ratio(nodays), &
                 ...
                 Reff_time(nodays),LWP_time(nodays))
...
!    rfac = 0.5d0 ; rfac(5) = 0.1d0 ; rfac(6) = 0d0
    ...
    snow_storage_time(n) = snow_storage
       ...
       wSWP_time(n) = wSWP ; rSWP_time(n) = rSWP ; root_depth_time(n) = root_reach; Reff_time(n) = Reff
       ...
       mLWP =  wSWP - (((transpiration*dayl_seconds_1)/mmol_to_kg_water) * Reff)
       LWP_time(n) = mLWP
       ...
       ! litter to som
       FLUXES(n,15) = POOLS(n,5)*(1d0-(1d0-pars(1)*FLUXES(n,2))**days_per_step)/days_per_step
       ...
       ! labile pool
       POOLS(n+1,1) = POOLS(n,1) + (FLUXES(n,5)-FLUXES(n,8))*days_per_step
       ! foliar pool
...
!    endif
  ...
  subroutine calculate_Rtot
    ...
    ! reset water flux
    total_water_flux = 0d0 ; water_flux_mmolH2Om2s = 0d0 ; mLWP = 0d0; wSWP = 0d0 ; rSWP = 0d0
...
!    endif
  ...
  subroutine plant_soil_flow(root_layer,root_length,root_mass &
    ...
    if (demand > 0d0) then
      conductance_mmolH2OMPam2s(root_layer) = 1d0/Rtot_layer
    else
      conductance_mmolH2OMPam2s(root_layer) = 0d0
    endif
