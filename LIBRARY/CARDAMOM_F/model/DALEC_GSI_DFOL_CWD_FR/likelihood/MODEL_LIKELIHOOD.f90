...
!    endif
  ...
  subroutine EDC2_GSI(npars,nomet,nofluxes,nopools,nodays,deltat &
    ...
    double precision :: mean_pools(nopools), meangpp, infi, &
                        ...
                        init_dyn_window, tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, Rs, dble_nodays, &
...
!    end if
    ...
    if (EDC2 == 1 .or. DIAG == 1) then
        ...
        ! Determine the number of time steps over which to assess initial dynamics
!        if (no_years > 1) then
!            init_dyn_window = steps_per_year * 2d0
!        else
!            init_dyn_window = steps_per_year
!        endif
        ...
        ! Three numbrs are calculated:
        ! in/out for first year
        ! in/out for second year
        ! in/out for second until final year
...
!            abs(log(in_out_litwood_yr1)) > EQF2 .or. log(in_out_litwood) > EQF2) then
  ...
  double precision function likelihood(npars,pars)
    use cardamom_structures, only: DATAin
    use CARBON_MODEL_MOD, only: fire_loss_wood
