...
subroutine rdaleccdeanolitroot(output_dim,MTT_dim,SS_dim &
                            ,met,pars &
...
!endif
     ...
     out_var1(i,1:nodays,9)  = FLUXES(1:nodays,8)       ! labile to foliage (gC/m2/day)
     out_var1(i,1:nodays,10) = FLUXES(1:nodays,10)      ! foliage to litter (gC/m2/day)
     out_var1(i,1:nodays,11) = FLUXES(1:nodays,11)      ! wood + roots to som (gC /m2/day)
     ! C disturbance fluxes (gC/m2/day)
     out_var1(i,1:nodays,12) = FLUXES(1:nodays,18)      ! fire emission from labile (gC/m2/day)
     out_var1(i,1:nodays,13) = FLUXES(1:nodays,24)      ! fire induced litter from labile (gC/m2/day)
     out_var1(i,1:nodays,14) = FLUXES(1:nodays,19)      ! fire emission from foliage (gC/m2/day)
     out_var1(i,1:nodays,15) = FLUXES(1:nodays,25)      ! fire induced litter from foliage (gC/m2/day)
     out_var1(i,1:nodays,16) = FLUXES(1:nodays,20)      ! fire emission from fine roots and wood (gC/m2/day)
     out_var1(i,1:nodays,17) = FLUXES(1:nodays,26)      ! fire induced litter from fine roots + wood(gC/m2/day)
     out_var1(i,1:nodays,18) = FLUXES(1:nodays,21)      ! fire emission from litter + som (gC/m2/day)
     out_var1(i,1:nodays,19) = FLUXES(1:nodays,27)      ! fire induced litter from wood (gC/m2/day)
     ! C pools (gC/m2)
     out_var1(i,1:nodays,20) = POOLS(1:nodays,1)        ! labile (gC/m2)
     out_var1(i,1:nodays,21) = POOLS(1:nodays,2)        ! foliage (gC/m2)
     out_var1(i,1:nodays,22) = POOLS(1:nodays,3)        ! fine root + wood (gC/m2)
     out_var1(i,1:nodays,23) = POOLS(1:nodays,4)        ! litter + som (gC/m2)
     ...
     out_var3(i,4) = sum(FLUXES(:,10)+ &
                         FLUXES(:,24)+FLUXES(:,25)+FLUXES(:,26))! litter (foliage + roots) + som
