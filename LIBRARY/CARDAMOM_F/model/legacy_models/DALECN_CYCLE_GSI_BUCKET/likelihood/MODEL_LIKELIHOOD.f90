...
!    endif
  ...
  subroutine EDC2_CROP(npars,nomet,nofluxes,nopools,nodays,deltat &
    ...
    if ((EDC2 == 1 .or. DIAG == 1) .and. &
        linear_model_gradient(M_LAI(DATAin%laipts),DATAin%LAI(DATAin%laipts),DATAin%nlai) < 0.25 ) then
...
double precision, dimension((DATAin%nodays+1), DATAin%nopools) :: M_POOLS
    ...
    if (DATAin%nlai > 0) then
       ...
       do n = 1, DATAin%nlai
         ...
         if (M_LAI(dn) >= 0.) then
             ! note that division is the uncertainty
             tot_exp=tot_exp+(log(max(0.001,M_LAI(dn))/max(0.001,DATAin%LAI(dn)))/log(DATAin%LAI_unc(dn)))**2
         endif
       end do
       do n = 1, DATAin%nlai
         dn=DATAin%laipts(n)
