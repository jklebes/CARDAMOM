...
double *LAI, double *NEE, double *FLUXES, double *POOLS) {
    ...
    for ( n=0; n < nr; n++) {
        ...
        FLUXES[f+0]=ACM(gpppars,constants)*MET[m+11]; //JFE to scale GPP by evaporative stress
...
// calculate the S-based based limiting factor - low stress is limiting
        double Sfac;
        if (MET[m+11]>pars[25]) // S larger than parameter Smax
        {
            Sfac = 1.;
        }
        else if (MET[m+11]<pars[24]) // S lower than parameter Smin
        {
            Sfac = 0.;         
        }
        else
        {
            Sfac = (MET[m+11]-pars[24])/(pars[25]-pars[24]);
        }

// calculate the GSI and store it in the FLUXES array with ID 17
        FLUXES[f+28] = Tfac*PHOTOfac*Sfac;
