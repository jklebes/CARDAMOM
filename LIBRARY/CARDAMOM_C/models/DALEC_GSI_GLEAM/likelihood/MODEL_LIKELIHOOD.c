#include <math.h>
#include "EDCs.c"
#include "../src/DALEC_LU_FIRES_GSI_GLEAM.c" // model
#include "../src/DALEC_LU_FIRES_GSI_GLEAM_PARS.c" // parameter info
...
//    for (n=0; n<PI.npars; n++){
  //  printf("param value passed EDCs0  %f %f\n", PI.parini[0], PARS[0]);
    ...
    EDC=pow(EDC1_LU_FIRES_GSI_GLEAM(PARS, &EDCD, D.meantemp, D.meanrad), D.EDC);
//    for (n=0; n<PI.npars; n++){
...
//        for (n=0; n<PI.npars; n++){
       // printf("param value passed before EDCs2c %f %f\n", PI.parini[0], PARS[0]);
        ...
        // EDC2 check
        EDC=EDC2_LU_FIRES_GSI_GLEAM(PARS, D.MET, D.M_LAI, D.M_NEE, D.M_POOLS, D.M_FLUXES, PI, D, &EDCD, D.meantemp);
        EDC=pow(EDC,D.EDC);
...
double edc_model_likelihood(DATA D, PARAMETER_INFO PI, double *PARS)
{
    ...
    EDC=EDC1_LU_FIRES_GSI_GLEAM(PARS, &EDCD, D.meantemp, D.meanrad);

    // running model
    CARBON_MODEL(D.MET, PARS, D.deltat, D.nodays, D.LAT, D.M_LAI, D.M_NEE, D.M_FLUXES, D.M_POOLS);

    // EDC2 check and return detailed EDCD.PASSFAIL
    EDC=EDC*EDC2_LU_FIRES_GSI_GLEAM(PARS, D.MET, D.M_LAI, D.M_NEE, D.M_POOLS, D.M_FLUXES, PI, D, &EDCD, D.meantemp);
