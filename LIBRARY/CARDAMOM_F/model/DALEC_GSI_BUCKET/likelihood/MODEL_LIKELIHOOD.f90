...
!    endif
  ...
  subroutine EDC2_CROP(npars,nomet,nofluxes,nopools,nodays,deltat &
    ...
    ! if ((EDC2 == 1 .or. DIAG == 1) .and. &
    !     linear_model_gradient(DATAin%M_LAI(DATAin%laipts),DATAin%LAI(DATAin%laipts),DATAin%nlai)
    !     < 0d0 ) then
  ...
  subroutine EDC2_GSI(npars,nomet,nofluxes,nopools,nodays,deltat &
    ...
    hak = 0 ; resid_lab = 0d0
    resid_lab(1:nodays) = (M_FLUXES(1:nodays,8)+M_FLUXES(1:nodays,7)+M_FLUXES(1:nodays,6)) &
                        / M_POOLS(1:nodays,1)
    ! division by zero results in NaN plus obviously I can't have turned
    ! anything over if there was nothing to start out with...
    ...
    sumgpp = sum(M_GPP(1:nodays))**(-1d0)
    sumnpp = (sum(M_GPP(1:nodays))*(1d0-fauto))**(-1d0)
    ...
    resid_time = (torfol*365.25)**(-1d0)
    if ((EDC2 == 1 .or. DIAG == 1) .and. (resid_time > 8d0 .or. resid_time < 0.15d0) ) then
    ...
    resid_time = (torlab*365.25d0)**(-1d0)
    if ((EDC2 == 1 .or. DIAG == 1) .and. resid_time > 18d0 ) then
    ...
    if (EDC2 == 1 .or. DIAG == 1) then
        ...
        if ((sum(mean_ratio)/dble(nodays-sum(hak))) > 0.08d0 .or. maxval(mean_ratio) > 0.125d0) then
...
!print*,"exp = ",n
    ...
    ! than 15 m2/m2
    if ((EDC2 == 1 .or. DIAG == 1) .and. (maxval(M_POOLS(:,2))/pars(17)) > 15d0 ) then
...
!       EQF10=log(10d0)
  ...
  double precision function cal_mean_annual_pools(pools,year,pool_number,nopools,interval,averaging_period)
    ...
    startday=floor(365.25d0*(year-1)/(sum(interval)/(averaging_period-1)))+1
    endday=floor(365.25d0*year/(sum(interval)/(averaging_period-1)))
  ...
  double precision function expdecay2(pools,pool_number,interval,nopools,averaging_period)
   ...
   os = 1 ! offset in days
   aw = floor(365.25d0/(sum(interval)/(averaging_period-1))) ! averaging window
