...
submit_processes_to_local_machine<-function (PROJECT_in) {
    ...
    for (n in seq(1, PROJECT_in$nosites)) {
         current_time=floor(as.numeric(Sys.time())) ; not_time = TRUE
         for (c in seq(1, PROJECT_in$nochains)) {
              # make sure that we are submitting the next chain at a different time - as this impacts the seed value for the random number generator
              while (not_time) {
                    now_time = floor(as.numeric(Sys.time()))
                    if (now_time != current_time) {
                        current_time=floor(as.numeric(Sys.time()))
                        not_time = FALSE
                    } # are time different yet?
               } # is this the right time
               not_time = TRUE # reset for next iteration of the loop[A
