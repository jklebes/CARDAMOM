...
!    endif
  ...
  subroutine EDC2_GSI(npars,nomet,nofluxes,nopools,nodays,deltat &
    ...
    ! declare local variables
    logical :: found
    integer :: n, DIAG, no_years, y, PEDC, nn, nnn, num_EDC, i
    double precision :: mean_pools(nopools), meangpp, infi, &
                        sumgpp, sumnpp, temp_response,hold, steps_per_year, steps_per_month &
                       ,tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, Rs, dble_nodays
    ...
    resid_lab = (M_FLUXES(:,8)+Rg_from_labile+M_FLUXES(:,22)) &
                        / M_POOLS(1:nodays,1)
    ! division by zero results in NaN plus obviously I can't have turned
    ! anything over if there was nothing to start out with...
    ...
    ! number of years in analysis (0.002737851 = 365.25**-1)
...
!    endif
    ...
    ! Thus fail conditions fNPP + rNPP + wNPP > 1.0 .or. fNPP + rNPP + wNPP < 0.95, i.e. abs(lNPP) cannot be > 0.05
    tmp1 = 1d0 - rNPP - wNPP - fNPP
    if ((EDC2 == 1 .or. DIAG == 1) .and. abs(tmp1) > 0.05d0) then
        EDC2 = 0d0 ; EDCD%PASSFAIL(21) = 0
    endif
