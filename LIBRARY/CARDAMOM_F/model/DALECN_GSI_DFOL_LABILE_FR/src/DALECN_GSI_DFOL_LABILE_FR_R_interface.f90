...
!endif
     ...
     out_var(i,1:nodays,18) = POOLS(1:nodays,7) ! root labile pool
     out_var(i,1:nodays,19) = POOLS(1:nodays,8) ! wood labile
     out_var(i,1:nodays,20) = POOLS(1:nodays,9) ! CWD

     ! N related outputs
     out_var(i,1:nodays,21) = POOLS(1:nodays,11)
     out_var(i,1:nodays,22) = POOLS(1:nodays,10)
     out_var(i,1:nodays,23) = POOLS(1:nodays,12)
     ...
     ! then for fine root
     hak = 0
     resid_fol(1:nodays) = FLUXES(1:nodays,12)/POOLS(1:nodays,3)
     ! division by zero results in NaN plus obviously I can't have turned
     ! anything over if there was nothing to start out with...
...
end subroutine rdalecngsidfollabfr
