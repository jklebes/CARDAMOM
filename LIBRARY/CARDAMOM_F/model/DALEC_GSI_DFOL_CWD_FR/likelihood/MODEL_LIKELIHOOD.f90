...
!    endif
  ...
  subroutine assess_EDC2(npars,nomet,nofluxes,nopools,nodays,deltat &
    ...
    ! declare local variables
    logical :: found
    integer :: n, DIAG, no_years, y, PEDC, nn, nnn, num_EDC, i, io_start, io_finish, &
               steps_per_year, steps_per_month
    double precision :: mean_pools(nopools), meangpp, infi, &
                        sumgpp, sumnpp, temp_response,hold, &
...
!        end if
  ...
  subroutine model_likelihood(PARS,ML_obs_out,ML_prior_out)
    ...
    if (DATAin%EDC == 1) then
        ...
        ! call EDCs which can be evaluated prior to running the model
        call assess_EDC1_GSI(PARS,PI%npars,DATAin%meantemp, DATAin%meanrad,EDC1)
