...
subroutine rdalecgsibucket(output_dim,aNPP_dim,met,pars,out_var,out_var2,lat &
...
!endif
     ...
     if (pft == 1) then
        out_var(i,1:nodays,14) = 0.0
        out_var(i,1:nodays,15) = 0.0 ! GSI temp component
        out_var(i,1:nodays,16) = 0.0 ! GSI photoperiod component
     ...
     if (pft == 1) then
        ...
        out_var(i,1:nodays,22) = 0.0 ! no Cauto pool present
     endif
     ! output fire
