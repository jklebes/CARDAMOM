...
! of DALEC.C4.D1.F2 as a function of observations and ecological dynamical constraints.
  !
...
!         tot_exp = tot_exp+(log(M_POOLS(dn,1)/DATAin%Cfol_stock(dn))/log(2d0))**2d0
         tot_exp = tot_exp+((M_POOLS(dn,1)-DATAin%Cfol_stock(dn)) / DATAin%Cfol_stock_unc(dn))**2d0
       end do
       likelihood = likelihood-tot_exp
    endif
    ...
    if (DATAin%nCfolmax_stock > 0) then
       ...
       do y = 1, no_years
          ...
          mean_annual_pools(y) = cal_max_annual_pools(M_POOLS(1:(DATAin%nodays+1),1),y,DATAin%deltat,DATAin%nodays+1)
       end do ! year loop
    ...
    if (DATAin%nCwood_stock > 0) then
       ...
       do n = 1, DATAin%nCwood_stock
         ...
         ! note that division is the uncertainty
!         tot_exp=tot_exp+(log(M_POOLS(dn,3)/DATAin%Cwood_stock(dn))/log(2d0))**2d0
         tot_exp = tot_exp+((M_POOLS(dn,3)-DATAin%Cwood_stock(dn))/DATAin%Cwood_stock_unc(dn))**2d0
       end do
       likelihood = likelihood-tot_exp
    endif
    ...
    if (DATAin%nCroots_stock > 0) then
       ...
       do n = 1, DATAin%nCroots_stock
         ...
         ! note that division is the uncertainty
!         tot_exp=tot_exp+(log(M_POOLS(dn,2)/DATAin%Croots_stock(dn))/log(2d0))**2d0
         tot_exp = tot_exp+((M_POOLS(dn,2)-DATAin%Croots_stock(dn)) / DATAin%Croots_stock_unc(dn))**2d0
       end do
       likelihood = likelihood-tot_exp
    endif
    ...
    if (DATAin%nClit_stock > 0) then
       ...
       do n = 1, DATAin%nClit_stock
         ...
         tot_exp = tot_exp+(((sum(M_FLUXES(:,10))/sum(M_FLUXES(:,10)+M_FLUXES(:,12))) &
                           *(M_POOLS(dn,4))-DATAin%Clit_stock(dn))/DATAin%Clit_stock_unc(dn))**2d0
       end do
       likelihood = likelihood-tot_exp
    endif
    ...
    if (DATAin%nCsom_stock > 0) then
       ...
       do n = 1, DATAin%nCsom_stock
         ...
         ! note that division is the uncertainty
!         tot_exp=tot_exp+(log(M_POOLS(dn,5)/DATAin%Csom_stock(dn))/log(2d0))**2d0
         tot_exp = tot_exp+((M_POOLS(dn,5)-DATAin%Csom_stock(dn))/DATAin%Csom_stock_unc(dn))**2d0
       end do
       likelihood = likelihood-tot_exp
    endif
...
!         tot_exp=tot_exp+(log(M_POOLS(dn,5)/DATAin%Csom_stock(dn))/log(2d0))**2d0
    ...
    if (DATAin%otherpriors(1) > 0) then
        tot_exp = sum(M_FLUXES(:,3)) / sum(M_FLUXES(:,1))
        sqrt_scale_likelihood = sqrt_scale_likelihood-((tot_exp-DATAin%otherpriors(1))/DATAin%otherpriorunc(1))**2d0
    end if
...
!         tot_exp=tot_exp+(log(M_POOLS(dn,5)/DATAin%Csom_stock(dn))/log(2d0))**2d0
    ...
    if (DATAin%otherpriors(1) > 0) then
        tot_exp = sum(M_FLUXES(:,3)) / sum(M_FLUXES(:,1))
        log_scale_likelihood = log_scale_likelihood-((tot_exp-DATAin%otherpriors(1))/DATAin%otherpriorunc(1))**2d0
    end if
