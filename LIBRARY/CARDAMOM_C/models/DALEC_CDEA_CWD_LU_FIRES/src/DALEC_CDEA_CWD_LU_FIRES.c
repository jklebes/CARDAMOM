...
// JFE 8 May 1027 - added a CWD pool to solve pb with tropical forests that accumulate a lot of C in soils
...
double *LAI, double *NEE, double *FLUXES, double *POOLS) {
    ...
    16.cwd to litter - May 2017 - JFE

    17.total release by fire - Jun 2014 / added by JFE 

    18-24: release by fire for each pool
    25-31: fluxes between pools
   ...
   // double NCFF[5];
    ...
    double CF[7] = {0.1,0.9,0.1,0.1,0.5,0.01,0.5};
    ...
    for ( n=0; n < nr; n++) {
       ...
       // printf("Adapting timestep %f", timestep);
        ...
        // total wood litter production aka wood to cwd  JFE - MAY 17
        ...
        // cwd to litter
        FLUXES[f+16] = POOLS[p+6]*(1-pow(1-FLUXES[f+1]*pars[24],timestep))/timestep;         
        ...
        POOLS[nxp+4] = POOLS[p+4] + (FLUXES[f+9] + FLUXES[f+11] + FLUXES[f+16] - FLUXES[f+12] - FLUXES[f+14])*timestep;                

        // SOM pool
        //POOLS[nxp+5]= POOLS[p+5]+ (FLUXES[f+14] - FLUXES[f+13]+FLUXES[f+10])*deltat;
        POOLS[nxp+5]= POOLS[p+5]+ (FLUXES[f+14] - FLUXES[f+13])*timestep;

        // CWD pool - add wood litter prod and remove transfer to litter
        POOLS[nxp+6] = POOLS[p+6] + (FLUXES[f+10]-FLUXES[f+16])*timestep;
      ...
      //      printf("End deforestation %03i\n %f\n", n, POOLS[nxp+0]);
        ...
        if (MET[m+7] > 0.) 
        {
         ...
         //   printf("Fire detected\n %f\n", POOLS[nxp+0]);
	        ...
	        for (nn=0;nn<7;nn++){FLUXES[f+18+nn] = POOLS[nxp+nn]*MET[m+7]*CF[nn]/timestep;}
	        for (nn=0;nn<7;nn++){FLUXES[f+25+nn] = POOLS[nxp+nn]*MET[m+7]*(1-CF[nn])*(1-rfac)/timestep;}
            //no litter transfer from soil
            FLUXES[f+25+5] = 0;

	        /*Adding all fire pool transfers here*/
	        /*live C pools*/	
	        for (nn=0;nn<4;nn++){POOLS[nxp+nn]=POOLS[nxp+nn]-(FLUXES[f+18+nn]+FLUXES[f+25+nn])*timestep;}
	        /*dead C pools*/
	        //litter: input from labile,foliar,root, output to soil
	        POOLS[nxp+4]=POOLS[nxp+4]+(FLUXES[f+25]+FLUXES[f+25+1]+FLUXES[f+25+2]-FLUXES[f+18+4]-FLUXES[f+25+4])*timestep;
	        // som: input from wood litter cwd
	        POOLS[nxp+5]=POOLS[nxp+5]+(FLUXES[f+25+3]+FLUXES[f+25+4]+FLUXES[f+25+6]-FLUXES[f+18+5])*timestep;
            //cwd: output to som
            POOLS[nxp+6]=POOLS[nxp+6]+(-FLUXES[f+18+6]-FLUXES[f+25+6])*timestep;

	        /*fires - total flux in gC m-2 day-1*/
	        /*this term is now (essentially) obsolete*/
	        /*replace in next version of DALEC_FIRES*/
	        for (nn=0;nn<7;nn++){FLUXES[f+17]+=FLUXES[f+18+nn];}
