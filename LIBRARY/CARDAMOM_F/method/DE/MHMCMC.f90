module DE_MODULE
  ...
  ! jklebes 2024
  ! with reference to spotpy and scipy implementation
    ! 
    ! To meet tests in test/method/DE
  !  
  !  Unlike scipy and like Cardamom MCMC:
  !  - bounds are hardcoded to [0,1] in all dimensions; we expect to 
  !    work with normalized parameters throughout
    ! 
    ! Basic implementation with limited features:
    ! - we have only one initialization option, it's 'random'
    ! - no hooks for callback or polish steps
    ! - we have only one choice of evolution fct
    ! - no constraints (other than bounds) implemented
    !
    ! - Still to add any parallelization; evaluated functions
    ! and updates candidates in series
  !!!!
  implicit none
