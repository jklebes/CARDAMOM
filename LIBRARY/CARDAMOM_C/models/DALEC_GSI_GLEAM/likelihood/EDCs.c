...
int EDC1_LU_FIRES_GSI_GLEAM(double const pars[], struct EDCDIAGNOSTIC *EDCD, double meantemp, double meanrad)
{
    ...
    //commented out to avoid problems with Sprox
    ...
    /*  GSI specific checks
        Tmn,min < Tmn,max */
    if ((EDC==1 || DIAG==1) && (pars[13]>pars[14])){EDC=0;EDCD->PASSFAIL[46]=0;}

    //  Photoperiod,min < Photoperiod,max
    if ((EDC==1 || DIAG==1) && (pars[15]>pars[23])){EDC=0;EDCD->PASSFAIL[47]=0;}

    //  VPD,min < VPD,max
    if ((EDC==1 || DIAG==1) && (pars[24]>pars[25])){EDC=0;EDCD->PASSFAIL[48]=0;}
...
int EDC2_LU_FIRES_GSI_GLEAM(double const pars[], double *MET, double *LAI, double *NEE, double *POOLS,double *FLUXES, PARAMETER_INFO PI, DATA D, struct EDCDIAGNOSTIC *EDCD, double meantemp)
{
...
double EQF=2;// JFE - put 2. following chat with AAB
double meangpp=0;
