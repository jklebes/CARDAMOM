  ...
  subroutine CARBON_MODEL(start,finish,met,pars,deltat,nodays,lat,lai_out,NEE_out,FLUXES,POOLS &
    ...
    if (.not.allocated(deltat_1)) then
        ...
        allocate(harvest_residue_to_litter(nodays),harvest_residue_to_som(nodays),         &
                 ...
                 root_depth_time(nodays),snow_storage_time(nodays))
...
!END!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ...
       wSWP_time(n) = wSWP ; deltaWP = min(0d0,minlwp-wSWP) ; root_depth_time(n) = root_reach
...
!       POOLS(n+1,7) = POOLS(n,7) + ((FLUXES(n,11)*(1d0-pars(29)))-FLUXES(n,20)-FLUXES(n,4))*deltat(n)
       ...
       FLUXES(n,26) = transpiration   ! transpiration (kgH2O/m2/day)
       FLUXES(n,27) = soilevaporation ! soil evaporation (kgH2O/m2/day)
       FLUXES(n,28) = wetcanopy_evap  ! wet canopy evaporation (kgH2O/m2/day)
       FLUXES(n,29) = runoff          ! soil surface runoff (kgH2O/m2/day)
       FLUXES(n,30) = underflow       ! drainage from bottom of soil column (kgH2O/m2/day)
       FLUXES(n,31) = water_grav_flow(1) ! drainage from the surface soil layer to 2nd (kgH2O/m2/day)
       FLUXES(n,32) = infiltrated(1)  ! top soil surface infiltration by rain (kgH2O/m2/day)
       FLUXES(n,35) = infiltrated(2)  ! middle soil surface infiltration by rain (kgH2O/m2/day)
       FLUXES(n,36) = infiltrated(3)  ! bottom soil surface infiltration by rain (kgH2O/m2/day)       
       FLUXES(n,33) = uptake_fraction(1) ! transpiration fraction extracted from 1st rooting layer (the soil surface)
       FLUXES(n,34) = uptake_fraction(2) ! transpiration fraction extracted from 2nd rooting layer (dynamic 2nd layer)
    ...
    if (foliage > 0d0) then
      ...
      do a = oldest_leaf, canopy_maturation_lag, -1
        ...
        if (canopy_age_vector(a) > 0d0) then
            ...
            if (stomatal_conductance > vsmall) then
                call acm_gpp_stage_1
                deltaGPP = acm_gpp_stage_2(stomatal_conductance) * umol_to_gC * dayl_seconds
            else
                deltaGPP = 0d0
            endif
...
!!!TLS: This should / could now be updated to bring in line with code in DALEC33 or DALEC23?
    ...
    if (avail_labile > 0d0 .and. GSI(current_step) > 0d0) then
        ...
        if (stomatal_conductance > vsmall) then
            call acm_gpp_stage_1
            old_GPP = acm_gpp_stage_2(stomatal_conductance) * umol_to_gC * dayl_seconds
        else
            old_GPP = 0d0
        endif
...
!        call calculate_stomatal_conductance
        ...
        if (stomatal_conductance > vsmall) then
             call acm_gpp_stage_1
            alt_GPP = acm_gpp_stage_2(stomatal_conductance) * umol_to_gC * dayl_seconds
        else
            alt_GPP = 0d0
        endif
