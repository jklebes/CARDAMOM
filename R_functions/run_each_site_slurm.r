...
if (length(args) == 4) {
    ...
    # Second should be the repair status of the task
    repair = as.numeric(args[2])
    # Third should be the site number from the project to start with
    s = as.numeric(args[3])
    # Fourth should be the site number from the project to finish with
    e = as.numeric(args[4])

    # The looping structure of the slurm script tries to 
    # run the largest number of sites per task. SO in the final task
    # there will be more requests than there are sites.
    # This guards against running sites that do not exist.
    e = min(e, PROJECT$nosites)
