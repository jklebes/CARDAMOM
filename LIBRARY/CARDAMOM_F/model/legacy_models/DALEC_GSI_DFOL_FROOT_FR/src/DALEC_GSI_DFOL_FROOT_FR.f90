...
!
  subroutine CARBON_MODEL(start,finish,met,pars,deltat,nodays,lat,lai,NEE,FLUXES,POOLS &
    ...
    double precision :: CFF(7) = 0, CFF_res(7) = 0    & ! combusted and non-combustion fluxes
                       ,NCFF(7) = 0, NCFF_res(7) = 0  & ! with residue and non-residue seperates
    ...
    ! local variables for two pool root model
    double precision :: aroot_litter,troot_litter,root_ratio,npp_root &
                       ,aroot_loss_rate,aroot_gain_rate
    ...
    if (start == 1) then
       ...
       disturbance_residue_to_litter = 0
       disturbance_residue_to_som = 0
       ...
       Cwood_labile_release_gradient = 0.25
       Cwood_labile_half_saturation = 20.0
    ...
    do n = start, finish
      ...
      npp_root    = (FLUXES(n,1)-FLUXES(n,3)-FLUXES(n,5))*pars(4)
      ! transport root production rate (gC.m-2.day-1)
      FLUXES(n,4) = npp_root*pars(38)
      ! absorpative root production (gC.m-2.day-1)
      FLUXES(n,6) = max(0.0,npp_root-FLUXES(n,4))
      ...
      ! labile allocation to wood
      npp_to_wood = FLUXES(n,1)-FLUXES(n,3)-FLUXES(n,5)-npp_root
      ! what is temperature limitation on wood labile release
      Cwood_labile_release_coef = 1.0 / &
                                 (1.0+exp(-Cwood_labile_release_gradient* &
                                 (((met(3,n)+met(2,n))*0.5)-Cwood_labile_half_saturation)))
      ! then impose secondary moisture stress.
      ! Currently assuming that if moisture stress on canopy is severe that wood
      ! growth is also already limited.
      Cwood_labile_release_coef = Cwood_labile_release_coef * VPDfac
      ! wood production
      FLUXES(n,7) = pars(44)*Cwood_labile_release_coef
      ...
      if (gradient < fol_turn_crit .or. FLUXES(n,18) == 0) then
         ...
         if (just_grown >= 1.0) then
            ...
            ! critical value, if not then no labile turnover allowed
            if ( ((tmp - FLUXES(n,1))/FLUXES(n,1)) < pars(27) ) then
                FLUXES(n,16) = 0d0 ; aroot_gain_rate = 0d0
            endif
         end if ! Just grown?
      ...
      !
      ! Update fine root absorptive root labile
      !

      ! fine root labile pool allocation
      FLUXES(n,19) = POOLS(n,8)*(1.-(1.-aroot_gain_rate)**deltat(n))/deltat(n)
      ! wood labile pool allocatin
      FLUXES(n,7) = POOLS(n,9)*(1.0-(1.0-FLUXES(n,7))**deltat(n))/deltat(n)
      ...
      ! absorptive root litter
      aroot_litter = POOLS(n,3)*(1.-(1.-aroot_loss_rate)**deltat(n))/deltat(n)
      ! transport root litter
      troot_litter = POOLS(n,7)*(1.-(1.-pars(37))**deltat(n))/deltat(n)
      ! total root litter production
      FLUXES(n,12) = aroot_litter + troot_litter
      ...
      ! absorptive labile pool
      POOLS(n+1,8) = POOLS(n,8) + (FLUXES(n,6)-FLUXES(n,19))*deltat(n)
      ! wood labile pool
      POOLS(n+1,9) = POOLS(n,9) + (npp_to_wood-FLUXES(n,7))*deltat(n)
      ! foliar pool
      POOLS(n+1,2) = POOLS(n,2) + (FLUXES(n,8) - FLUXES(n,10))*deltat(n)
      ! wood pool
      POOLS(n+1,4) = POOLS(n,4) + (FLUXES(n,7)-FLUXES(n,11))*deltat(n)
      ! absorptive root pool
      POOLS(n+1,3) = POOLS(n,3) + (FLUXES(n,19) - aroot_litter)*deltat(n)
      ! transport root pool
      POOLS(n+1,7) = POOLS(n,7) + (FLUXES(n,4) - troot_litter)*deltat(n)
      ! litter pool
      ...
      if (met(8,n) > 0.) then
          ...
          ! calculate ratio of absorptive:total_fine_root
          root_ratio = POOLS(n+1,3) / (POOLS(n+1,3)+POOLS(n+1,7))
          ...
          labile_frac_res = ( (POOLS(n+1,2)/C_total) * foliage_frac_res(harvest_management) ) &
                          + ( (POOLS(n+1,3)+POOLS(n+1,7)/C_total) * roots_frac_res(harvest_management)   ) &
          ...
          labile_root_loss = POOLS(n+1,8)*met(8,n)
          labile_wood_loss = POOLS(n+1,9)*met(8,n)
          foliar_loss = POOLS(n+1,2)*met(8,n)
          roots_loss  = POOLS(n+1,3)*met(8,n) + POOLS(n+1,7)*met(8,n)
          ...
          labile_root_residue = POOLS(n+1,8)*met(8,n)*labile_frac_res
          labile_wood_residue = POOLS(n+1,9)*met(8,n)*labile_frac_res
          foliar_residue = POOLS(n+1,2)*met(8,n)*foliage_frac_res(harvest_management)
          roots_residue  = (POOLS(n+1,3)+POOLS(n+1,7))*met(8,n)*roots_frac_res(harvest_management)
          ...
          POOLS(n+1,8) = max(0.,POOLS(n+1,8)-labile_root_loss)
          POOLS(n+1,9) = max(0.,POOLS(n+1,9)-labile_wood_loss)
          POOLS(n+1,2) = max(0.,POOLS(n+1,2)-foliar_loss)
          POOLS(n+1,3) = max(0.,POOLS(n+1,3)-(roots_loss*root_ratio))
          POOLS(n+1,7) = max(0.,POOLS(n+1,7)-(roots_loss*(1.0-root_ratio)))
          ...
          if (post_harvest_burn(harvest_management) > 0.) then
              ...
              !/*ROOTLABILE*/
              CFF(6) = POOLS(n+1,8)*post_harvest_burn(harvest_management)*combust_eff(1)
              NCFF(6) = POOLS(n+1,8)*post_harvest_burn(harvest_management)*(1-combust_eff(1))*(1-rfac)
              CFF_res(6) = labile_root_residue*post_harvest_burn(harvest_management)*combust_eff(1)
              NCFF_res(6) = labile_root_residue*post_harvest_burn(harvest_management)*(1-combust_eff(1))*(1-rfac)
              !/*WOODLABILE*/
              CFF(7) = POOLS(n+1,9)*post_harvest_burn(harvest_management)*combust_eff(1)
              NCFF(7) = POOLS(n+1,9)*post_harvest_burn(harvest_management)*(1-combust_eff(1))*(1-rfac)
              CFF_res(7) = labile_wood_residue*post_harvest_burn(harvest_management)*combust_eff(1)
              NCFF_res(7) = labile_wood_residue*post_harvest_burn(harvest_management)*(1-combust_eff(1))*(1-rfac)
              !/*foliar*/
              ...
              NCFF(3) = 0. !(POOLS(n+1,3)+POOLS(n+1,7))*post_harvest_burn(harvest_management)*(1-combust_eff(3))*(1-rfac)
          ...
          POOLS(n+1,8) = max(0., POOLS(n+1,8) - CFF(6) - NCFF(6) )
          POOLS(n+1,9) = max(0., POOLS(n+1,9) - CFF(7) - NCFF(7) )
          POOLS(n+1,2) = max(0., POOLS(n+1,2) - CFF(2) - NCFF(2) )
          POOLS(n+1,3) = max(0., POOLS(n+1,3) - (CFF(3) + NCFF(3))*root_ratio )
          POOLS(n+1,7) = max(0., POOLS(n+1,7) - (CFF(3) + NCFF(3))*(1.0-root_ratio) )
      ...
      if (met(9,n) > 0.) then
         ...
         !/*ROOTLABILE*/
         CFF(6) = POOLS(n+1,8)*met(9,n)*combust_eff(1)
         NCFF(6) = POOLS(n+1,8)*met(9,n)*(1-combust_eff(1))*(1-rfac)
         !/*WOODLABILE*/
         CFF(7) = POOLS(n+1,9)*met(9,n)*combust_eff(1)
         NCFF(7) = POOLS(n+1,9)*met(9,n)*(1-combust_eff(1))*(1-rfac)
         !/*foliar*/
         CFF(2) = POOLS(n+1,2)*met(9,n)*combust_eff(2)
         NCFF(2) = POOLS(n+1,2)*met(9,n)*(1-combust_eff(2))*(1-rfac)
         !/*root*/
         CFF(3) = 0. ! (POOLS(n+1,3)+POOLS(n+1,7))*met(9,n)*combust_eff(3)
         NCFF(3) = 0. ! (POOLS(n+1,3)+POOLS(n+1,7))*met(9,n)*(1-combust_eff(3))*(1-rfac)
         !/*wood*/
         ...
         POOLS(n+1,8)=POOLS(n+1,8)-CFF(6)-NCFF(6)
         POOLS(n+1,9)=POOLS(n+1,9)-CFF(7)-NCFF(7)
         POOLS(n+1,2)=POOLS(n+1,2)-CFF(2)-NCFF(2)
         POOLS(n+1,3)=POOLS(n+1,3)-(CFF(3)+NCFF(3))*root_ratio
         POOLS(n+1,7)=POOLS(n+1,7)-(CFF(3)+NCFF(3))*(1.0-root_ratio)
