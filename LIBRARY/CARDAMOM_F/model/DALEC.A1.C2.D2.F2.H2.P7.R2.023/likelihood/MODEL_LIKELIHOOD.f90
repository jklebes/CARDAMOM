...
! of DALEC.A1.C2.D2.F2.H2.P7.R2 as a function of observations and ecological dynamical constraints.
  !
...
!    endif
  ...
  subroutine assess_EDC2(npars, nomet, nofluxes, nopools, nodays, deltat &
    ...
    integer:: y, n, DIAG, no_years, nn, nnn, num_EDC, i, io_start, io_finish, &
               steps_per_year, steps_per_month, fl, fs
...
!    end do  ! year loop
    ...
    do n = 1, nopools-1  ! miss out the surface soil water pool
...
!    endif
    ...
    ! The initial leaf life span parameter (pars(36)) should not be too far away from the
    ! actual estimate. Note torfol (frac/day) converted to days
    tmp = torfol**(-1d0); tmp1 = min(100d0, dble(10*no_years))
    if ((EDC2 == 1 .or. DIAG == 1) .and. (tmp > pars(36)+tmp1 .or. tmp < pars(36)-tmp1) ) then
...
!           FT(fl) = sum(M_FLUXES(1:nodays, fl)*deltat(1:nodays))
        ...
        Fin(8)  = FT(47) 
        Fout(8) = FT(42)+FT(41)+FT(46) 
        Fin_yr1(8)  = FT_yr1(47) 
        Fout_yr1(8) = FT_yr1(42)+FT_yr1(41)+FT_yr1(46) 
!        Fin_yr2(8)  = FT_yr2(47)
!        Fout_yr2(8) = FT_yr2(42)+FT_yr2(41)+FT_yr2(46)
...
!print*,"likelihood: Fire done"
    ...
    if (DATAin%otherpriors(5) > -9998) then
        ...
        output = sum(M_POOLS(:,4) / (M_FLUXES(:,11)+M_FLUXES(:,24)+M_FLUXES(:,31)))
    ...
    if (DATAin%otherpriors(6) > -9998) then
        ! Estimate ratio
        tot_exp = sum(Resp_leaf) / sum(M_FLUXES(:,3))
