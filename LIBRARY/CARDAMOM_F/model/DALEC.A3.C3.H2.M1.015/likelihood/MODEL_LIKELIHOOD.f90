...
!    endif
    ...
    if (DATAin%nharvest > 0) then
      ...
      do n = 1, DATAin%nharvest
        ...
        ! Estimate the mean allocation to wood over the lag period
        tmp_var = sum(M_FLUXES(s:dn, 21)) / DATAin%harvest_lag(dn)
...
!    end if
    ...
    if (DATAin%otherpriors(8) > -9998) then
        ...
        tot_exp = sum(M_FLUXES(:,21)*mid_state) / sum(M_FLUXES(:,1)*mid_state)
...
!    endif
    ...
    if (DATAin%nlai > 0) then
        ...
        tot_exp = sum(((M_LAI(DATAin%laipts(1:DATAin%nlai))-DATAin%LAI(DATAin%laipts(1:DATAin%nlai))) &
                       /DATAin%LAI_unc(DATAin%laipts(1:DATAin%nlai)))**2)
        if (minval(M_LAI) < 0d0) tot_exp = tot_exp + (-log(infini))
