#include <math.h>
#include "EDCs.c"
#include "../src/DALEC_CDEA_LU_FIRES_ET.c" // model
#include "../src/DALEC_CDEA_LU_FIRES_ET_PARS.c" // parameter info
...
//    for (n=0; n<PI.npars; n++){
  //  printf("param value passed EDCs0  %f %f\n", PI.parini[0], PARS[0]);
    ...
    EDC=pow(EDC1_LU_FIRES_ET(PARS, &EDCD, D.meantemp, D.meanrad), D.EDC);
//    for (n=0; n<PI.npars; n++){
...
//        for (n=0; n<PI.npars; n++){
       // printf("param value passed before EDCs2c %f %f\n", PI.parini[0], PARS[0]);
        ...
        // EDC2 check
        EDC=EDC2_LU_FIRES_ET(PARS, D.MET, D.M_LAI, D.M_NEE, D.M_POOLS, D.M_FLUXES, PI, D, &EDCD, D.meantemp);
        EDC=pow(EDC,D.EDC);
...
double edc_model_likelihood(DATA D, PARAMETER_INFO PI, double *PARS)
{
    ...
    EDC=EDC1_LU_FIRES_ET(PARS, &EDCD, D.meantemp, D.meanrad);
   ...
   // printf("EDC1 %d\n",EDC);
    ...
    EDC=EDC*EDC2_LU_FIRES_ET(PARS, D.MET, D.M_LAI, D.M_NEE, D.M_POOLS, D.M_FLUXES, PI, D, &EDCD, D.meantemp);
  //  printf("EDC2 %d\n",EDC);
