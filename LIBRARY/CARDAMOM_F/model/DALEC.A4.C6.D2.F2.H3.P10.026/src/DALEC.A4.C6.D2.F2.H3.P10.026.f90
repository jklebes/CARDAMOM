...
!                         iWUE = 4.6875d-4,     & ! Intrinsic water use efficiency (umolC/mmolH2O-1/m2leaf/s-1)
  ...
  type :: cohort_leaf_t
      integer :: age_days   = 0, &   ! days elapsed since birth
                 age_months = 0      ! integer months elapsed (age_days/30)
      double precision :: Cf           = 0d0, & ! leaf carbon (gC m-2)
                          ...
                          Pi_threshold = 0d0    ! economic shedding threshold (gC gC-1 d-1)
...
!    print*,"carbon_model: "
    ...
    iWUE = pars(44) ! load the inherent water use efficiency
...
!    rfac = 0.5d0 ; rfac(5) = 0.1d0 ; rfac(6) = 0d0
    ...
    do n = start, finish
       ...
       call compute_canopy_NUE_rel_age(max_leaf_cohorts,leaf_cohorts,canopy_NUE_rel,DIAGS(n,32),DIAGS(n,33))
       ...
       available_labile = POOLS(n,1) + ((FLUXES(n,5)-FLUXES(n,9)) * days_per_step)
       ! Determine the current daily minimum C return for a leaf cohort
       ncce_crit_foliar = pars(15) / pars(45)
       ! Do plant allocation
       call plant_allocation(nopools,days_per_step, &
                             ...
                             pars(33),ncce_crit_foliar,             & ! labile:biomass limitations
       ...
       call plant_natural_turnover(nodays, n, days_per_step,              & ! time related
                                   available_labile,                      & ! 
                                   ...
                                   DIAGS(n,26),DIAGS(n,27))                 ! combined index of NCCE and gradient, dNCCE_loss, NCCE_gradient
       ...
       if (FLUXES(n,4) > vsmall .and. &
           days_since_last_cohort >= cohort_birth_period) then
           ! Birth new cohort
           call create_cohort(FLUXES(n,4)*one_Rg_fraction*days_per_step, & ! Allocation that makes it to leaves
                              FLUXES(n,4)*days_per_step, &                 ! Total allocation costs
                              pars(15), pars(45))
           days_since_last_cohort = 0d0
       else
           ! Inter-birth step: add allocation to current cohort
           if (FLUXES(n,4) > vsmall) then
               call add_foliage_to_cohort(FLUXES(n,4)*one_Rg_fraction*days_per_step, & ! Allocation that makes it to leaves
                                          FLUXES(n,4)*days_per_step, &                 ! Total allocation costs
                                          pars(15), pars(45))
           end if
       end if
       ...
       call update_and_shed_cohorts(days_per_step, FLUXES(n,1), POOLS(n,2), &
                                    pars(45), pars(46), pars(47), pars(17), &
                                    pars(15), FLUXES(n,10), cohort_litter_flux, &
                                    resorb_flux, env_litter_out)

       ! cohort_litter_flux contains both environmental and economic litter.
       ! Replaces FLUXES(n,10) directly â€” environmental loss has been explicitly
       ...
       DIAGS(n,34) = dble(n_profitable_cohorts)
       ...
       tmp = POOLS(n,5)*(1d0-(1d0-FLUXES(n,2)*pars(8))**days_per_step)*days_per_step_1
       ...
       FLUXES(n,14) = POOLS(n,6)*(1d0-(1d0-FLUXES(n,2)*pars(9))**days_per_step)*days_per_step_1
       ...
       POOLS(n+1,1) = POOLS(n,1) + (FLUXES(n,5)+FLUXES(n,52)-FLUXES(n,4)-FLUXES(n,6)-FLUXES(n,7)- &
                                    FLUXES(n,9)-FLUXES(n,8))*days_per_step

       POOLS(n+1,1) = max(0d0,POOLS(n+1,1))
       ! foliar pool
       POOLS(n+1,2) = POOLS(n,2) + (FLUXES(n,4)-FLUXES(n,10))*days_per_step
       ...
       call rescale_cohort_cf(POOLS(n+1,2))
       ! wood pool
       ...
       ! som pool
       POOLS(n+1,6) = POOLS(n,6) + (FLUXES(n,15)-FLUXES(n,14)+FLUXES(n,11))*days_per_step
       ...
       FLUXES(n,44) = runoff          ! soil surface runoff (kgH2O/m2/day)
       FLUXES(n,45) = underflow       ! drainage from bottom of soil column (kgH2O/m2/day)
       FLUXES(n,46) = water_grav_flow(1) ! drainage from the surface soil layer to 2nd (kgH2O/m2/day)
       ...
       FLUXES(n,51) = infiltrated(3)  ! bottom soil surface infiltration by rain (kgH2O/m2/day)
       FLUXES(n,48) = uptake_fraction(1) ! transpiration extracted fraction from 1st rooting layer (the soil surface)
       FLUXES(n,49) = uptake_fraction(2) ! transpiration extracted fraction from 2nd rooting layer (dynamic 2nd layer)
       ...
       if (met(8,n) > 0d0) then
           ...
           if (C_total > 0d0) then
               ...
               FLUXES(n,31) = (labile_loss-labile_residue) * days_per_step_1  ! Labile extraction
               FLUXES(n,32) = (foliar_loss-foliar_residue) * days_per_step_1  ! foliage extraction
               FLUXES(n,33) = (roots_loss-roots_residue) * days_per_step_1    ! fine roots extraction
               FLUXES(n,34) = (wood_loss-wood_residue) * days_per_step_1      ! wood extraction
               FLUXES(n,35) = 0d0 ! litter extraction
               FLUXES(n,36) = soil_loss_with_roots * days_per_step_1          ! som extraction
               ! Convert harvest related residue generations to daily rate for output
               FLUXES(n,37) = labile_residue * days_per_step_1 ! labile residues
               FLUXES(n,38) = foliar_residue * days_per_step_1 ! foliage residues
               FLUXES(n,39) = roots_residue * days_per_step_1  ! fine roots residues
               FLUXES(n,40) = wood_residue * days_per_step_1   ! wood residues
       ...
       if (met(9,n) > 0d0 .or.(met(8,n) > 0d0 .and. harvest_management > 0)) then
           ...
           if (burnt_area > 0d0) then
               ...
               FLUXES(n,18) = POOLS(n+1,1)*burnt_area*cf(1)*days_per_step_1 ! labile
               FLUXES(n,19) = POOLS(n+1,2)*burnt_area*cf(2)*days_per_step_1 ! foliar
               FLUXES(n,20) = POOLS(n+1,3)*burnt_area*cf(3)*days_per_step_1 ! roots
               FLUXES(n,21) = POOLS(n+1,4)*burnt_area*cf(4)*days_per_step_1 ! wood
               FLUXES(n,22) = POOLS(n+1,5)*burnt_area*cf(5)*days_per_step_1 ! litter
               FLUXES(n,23) = POOLS(n+1,6)*burnt_area*cf(6)*days_per_step_1 ! som

               ! second calculate litter transfer fluxes in g C m-2 d-1, all pools except som
               FLUXES(n,24) = POOLS(n+1,1)*burnt_area*(1d0-cf(1))*(1d0-rfac(1))*days_per_step_1 ! labile into litter
               FLUXES(n,25) = POOLS(n+1,2)*burnt_area*(1d0-cf(2))*(1d0-rfac(2))*days_per_step_1 ! foliar into litter
               FLUXES(n,26) = POOLS(n+1,3)*burnt_area*(1d0-cf(3))*(1d0-rfac(3))*days_per_step_1 ! roots into litter
               FLUXES(n,27) = POOLS(n+1,4)*burnt_area*(1d0-cf(4))*(1d0-rfac(4))*days_per_step_1 ! wood into som
               FLUXES(n,28) = POOLS(n+1,5)*burnt_area*(1d0-cf(5))*(1d0-rfac(5))*days_per_step_1 ! litter into som
               ...
               POOLS(n+1,6) = POOLS(n+1,6) + (FLUXES(n,27) + FLUXES(n,28) - FLUXES(n,23)) * days_per_step
...
!    print*,"carbon_model: done"
  ...
  subroutine acm_gpp_stage_1
    ...
    leafT_freeze = leafT + freeze
    dT_Vc_minT = leafT - Vc_minT
    ...
    metabolic_limited_photosynthesis = Vcmax_ref * canopy_NUE_rel * leaf_canopy_light_scaling &
                                     * airt_adj
    ...
    Jmax = dayl_hours_fraction*2.787095d0*((Vcmax_ref*canopy_NUE_rel)**0.890d0) * leaf_canopy_light_scaling &
...
!  end function find_gs_WUE
  ...
  subroutine calculate_stomatal_conductance
    ...
    if (aerodynamic_conductance > vsmall .and. total_water_flux > vsmall .and. &
        ...
        iWUE_upper = find_gs_iWUE(potential_conductance)
        ...
        canopy_lwrad_Wm2 = canopy_lwrad_Wm2 - (4d0 * Tdiff * emiss_boltz * dayT**(3d0))
...
!    soil_reflectance(2)     = rad_pars(6) ! soil_par_reflectance
    ...
    ! For clarity and visual similarity, I've used the capital letter O
    O1 = 0.5d0-(0.633d0*leaf_distribution_deviance)-(0.33d0*leaf_distribution_deviance**2d0)
    ...
    leaf_angle = 0.5235988d0 !30d0 * (pi/180d0)
    ...
    ! Note that notations from Sellers (1985) have been given double letters if only single character was used,
    ! or written word for greek notation
    hh = (((bb**2d0 - cc**2d0))**(0.5d0)) / mu_obar
...
!(((rmax / rd) - 1)**(1/c)) * d50 = d ! Depth at which rd = 99 %
    ...
    do i = 1, nos_root_layers
       ...
       cumulative_depth = cumulative_depth + layer_thickness(i)
    ...
    do while (d < 4 .and. maxval(avail_to_flow) > vsmall)
        ...
        do s = 1, nos_soil_layers
           ...
           if (avail_to_flow(s) > 0d0 .and. soil_waterfrac(s+1) < porosity(s+1)) then
               ...
               call calculate_soil_conductivity(s,liquid,pot_drainage_k1)
               ...
               liquid = (soil_waterfrac_local(s) * liquid_fraction(s) )
               call calculate_soil_conductivity(s,liquid,pot_drainage_k2)
               ...
               ! Estimate the soil water conductance at this new state
               liquid = (soil_waterfrac_local(s) * liquid_fraction(s) )
               call calculate_soil_conductivity(s,liquid,pot_drainage_k3)
               ...
               ! Estimate the soil water conductance at this new state
               liquid = (soil_waterfrac_local(s) * liquid_fraction(s) )
               call calculate_soil_conductivity(s,liquid,pot_drainage_k4)
...
!    endif
  ...
  subroutine plant_allocation(nopools,time,                              &
                              ...
                              foliar_limit,ncce_gCm2day,delta_ncce_gCgC)   ! combined foliar limit, NCCE and dNCCE
       ...
       if (available_labile > 0d0) then
           ...
           if (alloc_leaf > 0d0) then
               ...
               lai_orig = lai ; scaling_orig = leaf_canopy_light_scaling
               gs_orig = stomatal_conductance ; NUE_rel_orig = canopy_NUE_rel
               ...
               call new_leaf_update_canopy_NUE_rel(foliage,(leaf_investment * one_Rg_fraction),1d0,canopy_NUE_rel)
               ...
               lai = lai_orig ; stomatal_conductance = gs_orig ; NUE_rel_orig = canopy_NUE_rel
               ! Update the shortwave radiation
               call calculate_shortwave_balance
               ! Update acm_gpp_stage_1
               call acm_gpp_stage_1
  ...
  subroutine plant_natural_turnover(nodays,step,time,                       & ! time related
                                    ...
                                    cmi,ncce_gradient)                        ! CMI, NCCE change for canopy loss, NCCE gradient
       ...
       if (foliage > 0d0) then
           ...
           if (available_labile <= 0d0) then
               ...
               C_starvation_loss = Rm_heskel_polynomial(Rm_leaf_const, leafT, cold_shutdown) * umol_to_gC * seconds_per_day
               ...
               C_starvation_loss = (abs(available_labile) / C_starvation_loss) * days_per_step_1
           else 
...
!    logistic_func = (1d0+exp(max_growth*(max_growth_point-invar)))**(-1d0)
  ...
  subroutine initialise_cohorts(foliage, start_doy, r_opp, leaf_age_ref, &
    ...
    do k = 1, max_leaf_cohorts
        leaf_cohorts(k)%age_days     = 0   ! days elapsed since birth
        leaf_cohorts(k)%age_months   = 0   ! integer months elapsed (age_days/30)
        leaf_cohorts(k)%Cf           = 0d0 ! leaf carbon (gC m-2)
        leaf_cohorts(k)%Cc           = 0d0 ! construction cost at birth (gC m-2)
        leaf_cohorts(k)%Pi_threshold = 0d0 ! economic shedding threshold (gC gC-1 d-1)
        leaf_cohorts(k)%cum_profit   = 0d0 ! cumulative NCCE since birth (gC m-2)
        leaf_cohorts(k)%NUE_rel      = 1d0 ! relative NUE content (0-1), declines with age
        leaf_cohorts(k)%is_alive     = .false.
    end do
    n_live_cohorts         = 0   ! No living cohorts yet
    newest_cohort_slot     = 0   ! no cohort born yet for this MCMC sample
    ...
    do y = 1, N_years
        ...
        ! Linear annual weight: w(y) = 2*y / (N_years*(N_years+1))
        ...
        do m = 1, 12
            ...
            ! it belongs to the previous year â€” add another year's worth of days.
            ...
            leaf_cohorts(k)%cum_profit  = 0d0 ! TLS: May need to initialise this also with a real value.
  ...
  !------------------------------------------------------------------
  !
  subroutine create_cohort(dCf, dCc, r_opp, leaf_age_ref)
    ...
    slot = 0
    do k = 1, max_leaf_cohorts
        if (.not. leaf_cohorts(k)%is_alive) then
            slot = k
            exit
        end if
    end do
    ...
    if (slot > 0) then
        ...
        leaf_cohorts(slot)%Cf          = dCf ! Actual leaves
        leaf_cohorts(slot)%Cc          = dCc ! Total construction cost
        leaf_cohorts(slot)%Pi_threshold= dCc * r_opp / max(leaf_age_ref, 1d0)
        leaf_cohorts(slot)%age_days    = 0
        leaf_cohorts(slot)%age_months  = 0
        leaf_cohorts(slot)%cum_profit  = -dCc ! start in construction debt
        leaf_cohorts(slot)%NUE_rel     = 1d0  ! fresh leaves
        leaf_cohorts(slot)%is_alive    = .true. ! IT'S ALIVE
        n_live_cohorts = n_live_cohorts + 1
  ...
  !------------------------------------------------------------------
  !
  subroutine add_foliage_to_cohort(dCf, dCc, r_opp, leaf_age_ref)
    ...
    if (newest_cohort_slot == 0) return
  ...
  subroutine update_and_shed_cohorts(time, gpp_gCm2day, foliage, &
    ...
    !
    ! Pass structure:
    !   Pass 1  â€” forward scan: age update, and update oldest cohort.
    !   Pass 2a â€” insertion sort of oldest_idx by descending age_days.
    !   Pass 2b â€” environmental loss: remove env_litter_gCm2 from oldest
    !             cohorts first. Partial removal supported; cohorts not killed
    !             unless Cf is fully exhausted.
    !   Pass 2c â€” economic shedding: remaining living cohorts.
    !
    ! Environmental loss field update rationale (Pass 2b):
    !   Cf           : reduced by delta_k (direct carbon removal).
    !   Pi_threshold : recomputed from UNCHANGED Cc. Cc records the total
    !                  construction investment committed since birth; environmental
    !                  loss does not reverse this investment. With unchanged Cc
    !                  and smaller Cf, Pi_threshold represents a higher effective
    !                  return requirement per unit remaining carbon â€” the correct
    ...
    !   Shed cohort k if: ncce_k < Pi_threshold(k)
    ...
    k_oldest = max(12, n_live_cohorts/2)
    ...
    ! -------------------------------------------------------------------------
    ! Precompute Rm per unit LAI [gC m-2leaf d-1] once â€” same for all cohorts
    ...
    ! -------------------------------------------------------------------------
    Rm_per_m2 = (Rm_heskel_polynomial(Rm_leaf_const, leafT, cold_shutdown) * umol_to_gC * seconds_per_day)
    ...
    ! -------------------------------------------------------------------------
    gpp_norm = canopy_NUE_rel * max(foliage, vsmall)
    ...
    ! =========================================================================
    ! PASS 1 â€” Profit update + oldest-k_oldest identification
    !
    ...
    do k = 1, max_leaf_cohorts
        ...
        if (n_oldest < k_oldest) then
            ! Phase A: set not full â€” insert unconditionally
            n_oldest = n_oldest + 1
            ...
            ! Phase B: set full â€” evict youngest if current cohort is older
        ...
        !
        ! Guard A: Cf(k) <= vsmall â€” cohort nearly exhausted by env loss;
        !          force shedding by setting ncce_k to large negative value.
        ! Guard B: gpp_norm <= vsmall â€” no canopy NUE or no foliar pool;
        ...
        if (leaf_cohorts(k)%Cf > vsmall) then
          ...
          if (gpp_norm > vsmall) then
            ...
            Rm_k   = (leaf_cohorts(k)%Cf/lca) * Rm_per_m2
            ! Determine cohort specific NCCE
            ncce_k = (gpp_k - Rm_k)
        else
           ncce_k = 0d0
        end if
        ...
        ! --- Age update ---
        leaf_cohorts(k)%age_days   = leaf_cohorts(k)%age_days + nint(time)
        leaf_cohorts(k)%age_months = leaf_cohorts(k)%age_days / 30
        ...
        leaf_cohorts(k)%NUE_rel = exp(-k_N_decline * dble(leaf_cohorts(k)%age_months))
        leaf_cohorts(k)%NUE_rel = max(0d0, min(1d0, leaf_cohorts(k)%NUE_rel))
        ...
        if (leaf_cohorts(k)%cum_profit > 0d0) n_profitable_cohorts = n_profitable_cohorts + 1

    end do ! k = 1, max_leaf_cohorts

    ! =========================================================================
    ! PASS 2 â€” Environmental loss then economic shedding, oldest-first
    !
    ! Step 2a: insertion sort of oldest_idx by descending age_days.
    ! Step 2b: environmental loss â€” remove env_litter_gCm2 from oldest cohorts.
    ! Step 2c: economic shedding â€” remaining living cohorts.
    ! =========================================================================
    ...
    ! --- Step 2b: environmental leaf loss â€” oldest cohorts first ---
    !
    ...
    !   Cf           : Cf -= delta_k
    !   Pi_threshold : recomputed from UNCHANGED Cc (Cc * r_opp / leaf_age_ref).
    !                  Unchanged Cc + smaller Cf â†’ higher effective requirement
    !                  per remaining carbon unit â€” correct economic interpretation.
    !   Cc           : UNCHANGED â€” construction cost already spent.
    !   cum_profit   : UNCHANGED â€” historical earnings not retroactively altered.
    !   NUE_rel, age_days, age_months : UNCHANGED â€” tissue properties, not mass.
    !
    ...
    if (remaining_env_loss > vsmall) then
        ...
        do j = 1, n_oldest
            ...
            delta_k = min(leaf_cohorts(k)%Cf, remaining_env_loss)
            ...
            leaf_cohorts(k)%Cf = leaf_cohorts(k)%Cf - delta_k
            ! Recompute Pi_threshold from UNCHANGED Cc:
        ...
        if (remaining_env_loss > vsmall) then
            ...
            do while (remaining_env_loss > vsmall)
                ...
                do k = 1, max_leaf_cohorts
                    if (.not. leaf_cohorts(k)%is_alive) cycle ! If dead
                    if (processed(k)) cycle ! If we've been before
                    if (leaf_cohorts(k)%age_days > next_oldest_age) then
                        next_oldest_age = leaf_cohorts(k)%age_days
                        next_oldest_k   = k
                    end if
                end do
...
!    end do ! Step 2c: remaining cohort economic shedding
  ...
  subroutine rescale_cohort_cf(target_Cf)
    ...
    !       The discrepancy is a few ULPs â€” proportional scaling distributes
    ...
    !   scale > 1 : net gain  (drift correction only â€” tiny)
    !   scale < 1 : net loss  (disturbance or drift correction)
    !   scale = 1 : no action (abs(delta_Cf) <= vsmall, return early)
    !
    ! Only Cf is scaled. Cc, Pi_threshold, and cum_profit are NOT updated here
  ...
  !------------------------------------------------------------------
  !
  subroutine compute_canopy_NUE_rel_age(n_cohorts,cohorts_in,canopy_NUE_rel,canopy_age_days,canopy_profit_gCm2)
    ...
    integer          :: k
    double precision :: sum_Cf, sum_NrCf, sum_AgCf, sum_PrCf

    ! Reset counters
    sum_Cf   = 0d0 ; sum_NrCf = 0d0 ; sum_AgCf = 0d0 ; sum_PrCf = 0d0
    ...
    if (sum_Cf > vsmall) then
        ...
        canopy_NUE_rel = 1d0 ; canopy_age_days = 0d0 ; canopy_profit_gCm2 = 0d0
    end if
