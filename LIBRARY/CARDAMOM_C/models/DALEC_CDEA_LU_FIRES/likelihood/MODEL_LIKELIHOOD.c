#include <math.h>
#include "EDCs.c"
#include "../src/DALEC_CDEA_LU_FIRES.c" // model
#include "../src/DALEC_CDEA_LU_FIRES_PARS.c" // parameter info
...
double likelihood(DATA D)
{
   ...
   // printf("%f\n",P);
    ...
    {        
        for (n=0;n<D.nCwood_stock;n++)
        {
            dn=D.Cwood_stockpts[n];
            double M_Cwood = D.M_POOLS[(dn*D.nopools)+3];
            tot_exp=tot_exp+pow((M_Cwood-D.Cwood_stock[dn])/(D.Cwood_stock[dn]*0.50),2);
        }
    }
  ...
  /* //Evap likelihood... use 25% uncertainty // COMMENTED OUT AS NO EVAP IN CDEA
    tot_exp=0;
    if (D.nevap>0)
    {
        for (n=0;n<D.nevap;n++)
        {
            dn=D.Evap_pts[n];
            double M_Evap = D.M_FLUXES[(dn*D.nofluxes)+28];
            tot_exp=tot_exp+pow((M_Evap-D.Evap[dn])/(D.Evap[dn]*0.25),2);
        }
    }
    P=P-0.5*tot_exp;
    if (isnan(P)){P=log(0);} 
...
//Csom likelihood
    ...
    {        
        for (n=0;n<D.nCsom_stock;n++)
        {
            dn=D.Csom_stockpts[n];
            double M_Csom = D.M_POOLS[(dn*D.nopools)+5];
            tot_exp=tot_exp+pow((M_Csom-D.Csom_stock[dn])/(D.Csom_stock[dn]*0.50),2);
        }
    }
    P=P-0.5*tot_exp;
    if (isnan(P)){P=log(0);} 
