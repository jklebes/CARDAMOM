...
subroutine rdaleccdeaacm2bucketrmheskelrgcwdwmrt(output_dim,aNPP_dim,MTT_dim,SS_dim,fire_dim &
                                                ,met,pars &
  ...
  use CARBON_MODEL_MOD, only: CARBON_MODEL, extracted_C, wSWP_time &
                             ...
                             ,gs_demand_supply_ratio, cica_time, Rm_from_labile &
                             ,Rg_from_labile &
...
!endif
     ...
     do y = 1, nos_years
        ...
        ! Estimate annual Mean residence time
        ...
        out_var7(i,5,y) = sum((FLUXES(y_s:y_e,22)+FLUXES(y_s:y_e,32))*deltat(y_s:y_e))
        ! Soil
        ...
        ! Litter (fol+fine root)
        out_var8(i,5,y) = sum((FLUXES(y_s:y_e,28)+FLUXES(y_s:y_e,33))*deltat(y_s:y_e))
        ! Soil
        out_var8(i,6,y) = 0d0
        ...
        ! Labile
        out_var9(i,1,y) = sum((FLUXES(y_s:y_e,8)+Rg_from_labile(y_s:y_e)+Rm_from_labile(y_s:y_e))*deltat(y_s:y_e))
        ! Foliage
        ...
        ! Litter (fol+fine root+wood)
        out_var9(i,5,y) = sum((FLUXES(y_s:y_e,13)+FLUXES(y_s:y_e,15)+FLUXES(y_s:y_e,30)+FLUXES(y_s:y_e,31))*deltat(y_s:y_e))
        ! Soil
...
end subroutine rdaleccdeaacm2bucketrmheskelrgcwdwmrt
