...
subroutine rdalecgsidfolcwdfr(output_dim,aNPP_dim,MTT_dim,SS_dim,met,pars,out_var,out_var2,out_var3,out_var4 &
                             ,lat &
...
!        out_var(i,1:nodays,14) = (sum(FLUXES(:,7)) / sum(FLUXES(:,11)+FLUXES(:,25))) &
!                               * (sum(POOLS(:,4)) / dble(nodays))
!        out_var(i,1:nodays,14) = ((out_var(i,1:nodays,14) - 8000d0) / 250d0) ** 2
...
!     out_var2(i,1) = sum(FLUXES(1:nodays,4)+FLUXES(1:nodays,8)) * sumNPP ! foliar
     ...
     if (pft == 1) then
         ...
         out_var3(i,1) = sum(resid_fol) / dble(nodays-sum(hak))
